...
import {
  getCanonicalCharacterSystemData,
  getCanonicalBestiarySystemData,
  getCanonicalVehicleSystemData,
} from "./canonical.mjs";
import {
  normalizeSkillEntry,
  normalizeSkillsData,
} from "./normalization-skills.mjs";
import {
...
export { normalizeChoiceGroup, choiceGroupToModifierGroup };
import {
...
const MYTHIC_NORMALIZATION_DEBUG = false;

const MYTHIC_BATTERY_SUBTYPES = Object.freeze(
  new Set(["plasma", "ionized-particle", "unsc-cell", "grindell"]),
);
...
function normalizeMedicalActiveEffectEntry(entry = {}, index = 0) {
  ...
  const requestedDomain =
    String(entry.domain ?? "medical")
      .trim()
      .toLowerCase() || "medical";
  const domain = ["medical", "environmental", "fear-ptsd"].includes(
    requestedDomain,
  )
    ? requestedDomain
    : "medical";
  ...
  const inferredHalfActions =
    Number.isFinite(durationHalfActions) && durationHalfActions > 0
      ? Math.max(0, Math.floor(durationHalfActions))
      : /half\s*actions?/iu.test(combinedDurationHints) && inferredCount > 0
        ? inferredCount
        : 0;
  const inferredRounds =
    Number.isFinite(durationRounds) && durationRounds > 0
      ? Math.max(0, Math.floor(durationRounds))
      : /\brounds?\b|\bturns?\b/iu.test(combinedDurationHints) &&
          inferredCount > 0
        ? inferredCount
        : 0;
  ...
  return {
    ...
    specialDamageValueRaw: Number.isFinite(specialDamageValueRaw)
      ? Math.max(0, Math.floor(specialDamageValueRaw))
      : 0,
...
function normalizeBatterySubtype(value = "", ammoMode = "") {
  ...
  if (["ionized", "ionized-particles", "ionized-particle"].includes(raw))
    return "ionized-particle";
  if (["unsc", "unsc-cell", "unsc-battery-cell"].includes(raw))
    return "unsc-cell";
...
const MYTHIC_VEHICLE_ARMOR_LOCATION_KEYS = Object.freeze([
  "front",
  "back",
  "side",
  "top",
...
const MYTHIC_VEHICLE_WALKER_LOCATION_KEYS = Object.freeze(
  MYTHIC_VEHICLE_WALKER_LOCATION_DEFINITIONS.map((entry) => entry.key),
);
const MYTHIC_VEHICLE_WALKER_ARM_TRACKER_PREFIX = "walker-arm-";
const MYTHIC_VEHICLE_WALKER_MAX_ARM_COUNT = 20;
const MYTHIC_VEHICLE_WALKER_PUNCH_MODIFIERS = Object.freeze([
  "none",
  "strength",
  "strength-x2",
]);
...
const MYTHIC_VEHICLE_DOOMED_DETONATION_MODES = Object.freeze([
  null,
  "tier7",
  "tier8",
  "instant",
...
function normalizeVehicleDoomedPersistentLedger(persistent = {}) {
  const source =
    persistent && typeof persistent === "object" && !Array.isArray(persistent)
      ? persistent
      : {};
  const requestedDamageMode = String(
    source.occupantsDamageMode ?? "none",
  ).trim();
  const occupantsDamageMode =
    MYTHIC_VEHICLE_DOOMED_OCCUPANT_DAMAGE_MODES.includes(requestedDamageMode)
      ? requestedDamageMode
      : "none";
  ...
  const detonationMode = MYTHIC_VEHICLE_DOOMED_DETONATION_MODES.includes(
    requestedDetonationMode,
  )
    ? requestedDetonationMode
    : null;
  const detonationRoundsRemaining =
  ...
  const detonationStartedAtRound =
...
function normalizeVehicleDoomCountdownState(countdown = {}) {
  const source =
    countdown && typeof countdown === "object" && !Array.isArray(countdown)
      ? countdown
      : {};
  return {
    ...
    detonationRound: Math.max(
      0,
      Math.floor(Number(source.detonationRound ?? 0) || 0),
    ),
    roundsRemaining: Math.max(
      0,
      Math.floor(Number(source.roundsRemaining ?? 0) || 0),
    ),
  };
}
...
function getVehicleBlastRadiusForDoomTier(
  doomedTier = 0,
  weaponPoints = 0,
  sizePoints = 0,
) {
  const safeWeaponPoints = Math.max(
    0,
    Math.floor(Number(weaponPoints ?? 0) || 0),
  );
  const safeSizePoints = Math.max(0, Math.floor(Number(sizePoints ?? 0) || 0));
  if (doomedTier === 7)
    return Math.floor((safeWeaponPoints + safeSizePoints) / 2);
  if (doomedTier === 8)
    return safeWeaponPoints + Math.floor(safeSizePoints / 2);
...
function getVehicleDoomState({
  ...
  const doomedArmorPenaltyFlat = isDoomed
    ? getVehicleDoomArmorPenaltyFlat(doomedTier)
    : 0;
  const normalizedPersistent =
    normalizeVehicleDoomedPersistentLedger(persistent);
  const normalizedLegacyCountdown =
    normalizeVehicleDoomCountdownState(countdown);
  const legacySource =
  ...
  const legacyRoundsRemaining = Math.max(
    0,
    Math.floor(Number(legacySource?.doomedDetonationRoundsRemaining ?? 0) || 0),
  );

  const nextPersistent = foundry.utils.deepClone(
    MYTHIC_VEHICLE_DOOMED_PERSISTENT_DEFAULTS,
  );
  nextPersistent.onFire =
    normalizedPersistent.onFire || legacySource?.doomedOnFire === true;
  nextPersistent.flameRating = Math.max(
    0,
    normalizedPersistent.flameRating,
    Number(legacySource?.doomedFlameRating ?? 0) || 0,
  );
  nextPersistent.occupantsDamageMode = normalizedPersistent.occupantsDamageMode;
  if (
    !MYTHIC_VEHICLE_DOOMED_OCCUPANT_DAMAGE_MODES.includes(
      nextPersistent.occupantsDamageMode,
    )
  ) {
    nextPersistent.occupantsDamageMode = "none";
  }
  nextPersistent.movementDisabled =
    normalizedPersistent.movementDisabled ||
    legacySource?.doomedVehicleImmobile === true;
  nextPersistent.engineAimingDisabled =
    normalizedPersistent.engineAimingDisabled ||
    legacySource?.doomedEngineAimingDisabled === true;
  nextPersistent.detonation = {
    armed:
      normalizedPersistent.detonation.armed ||
      normalizedLegacyCountdown.active === true,
    ...
    combatId:
      normalizedPersistent.detonation.combatId ||
      normalizedLegacyCountdown.combatId,
  };
  ...
  if (!nextPersistent.detonation.mode) {
    const legacyDetonationState = String(
      legacySource?.doomedDetonationState ?? "",
    )
      .trim()
      .toLowerCase();
    if (
      legacyDetonationState === "immediate" ||
      legacySource?.doomedImmediateDetonation === true
    )
      nextPersistent.detonation.mode = "instant";
    else if (legacyDetonationState === "countdown" && doomedTier >= 8)
      nextPersistent.detonation.mode = "tier8";
    else if (legacyDetonationState === "countdown" && doomedTier >= 7)
      nextPersistent.detonation.mode = "tier7";
  }

  if (!isDoomed) {
    const resetPersistent = normalizeVehicleDoomedPersistentLedger(
      foundry.utils.deepClone(MYTHIC_VEHICLE_DOOMED_PERSISTENT_DEFAULTS),
    );
    return {
      level: "tier_0",
      armor: 0,
      ...
      doomedEffectiveArmorByLocation: Object.fromEntries(
        MYTHIC_VEHICLE_ARMOR_LOCATION_KEYS.map((key) => {
          const baseArmor = Math.max(
            0,
            Math.floor(Number(baseArmorByLocation?.[key] ?? 0) || 0),
          );
      ...
      countdown: foundry.utils.deepClone(
        MYTHIC_VEHICLE_DOOM_COUNTDOWN_DEFAULTS,
      ),
    };
  }
  ...
  if (doomedTier >= 9) {
    ...
    nextPersistent.detonation.startedAtCombatRound = hasActiveCombat
      ? combatContext.round
      : nextPersistent.detonation.startedAtCombatRound;
    nextPersistent.detonation.combatId = hasActiveCombat
      ? combatContext.combatId
      : nextPersistent.detonation.combatId;
    ...
    if (!nextPersistent.detonation.armed) {
      ...
      if (currentRemaining === null || currentRemaining === undefined) {
        ...
        const resolvedRemaining = Math.max(
          0,
          Math.floor(Number(currentRemaining) || 0),
        );
        if (resolvedRemaining >= 3)
          nextPersistent.detonation.roundsRemaining = 2;
        else if (resolvedRemaining <= 0)
          nextPersistent.detonation.roundsRemaining = 0;
  ...
  if (nextPersistent.detonation.mode === "instant") {
    ...
    const storedRemaining =
      nextPersistent.detonation.roundsRemaining === null ||
      nextPersistent.detonation.roundsRemaining === undefined
        ? 0
        : Math.max(
            0,
  ...
  const doomedImmediateDetonation =
    nextPersistent.detonation.mode === "instant" ||
  ...
  const doomedEffectiveArmorByLocation = Object.fromEntries(
    MYTHIC_VEHICLE_ARMOR_LOCATION_KEYS.map((key) => {
      const baseArmor = Math.max(
        0,
        Math.floor(Number(baseArmorByLocation?.[key] ?? 0) || 0),
      ...
      const effectiveArmor = Math.max(
        0,
        Math.floor(baseArmor / 2) - doomedArmorPenaltyFlat,
      );
      return [key, effectiveArmor];
    }),
  );
  const doomedBlastRadius = getVehicleBlastRadiusForDoomTier(
    doomedTier,
    weaponPoints,
    sizePoints,
  );
  const doomedKillRadius =
    doomedBlastRadius > 0 ? Math.floor(doomedBlastRadius / 2) : 0;
  ...
  return {
    ...
    doomedFlameRating: nextPersistent.onFire
      ? Math.max(2, nextPersistent.flameRating)
      : 0,
    ...
    doomedDetonationRoundsRemaining: doomedImmediateDetonation
      ? 0
      : detonationRoundsRemaining,
    doomedDetonationSecondsRemaining: doomedImmediateDetonation
      ? 0
      : detonationRoundsRemaining * 6,
    doomedBlastRadius,
    doomedKillRadius,
    ...
    countdown: normalizeVehicleDoomCountdownState({
      ...
      detonationRound: 0,
      roundsRemaining: doomedImmediateDetonation
        ? 0
        : detonationRoundsRemaining,
    }),
  };
...
function normalizeVehiclePropulsionType(value = "wheels") {
  const normalized = String(value ?? "")
    .trim()
    .toLowerCase();
  if (normalized === "propeller" || normalized === "propellers")
    return "thrusters";
  if (["none", "legs", "thrusters", "treads", "wheels"].includes(normalized))
    return normalized;
  return "wheels";
}
...
function normalizeVehicleWalkerLocationState(location = {}, definition = {}) {
  ...
  return {
    ...
    breakpointType: String(
      definition?.breakpointType ?? source?.breakpointType ?? "",
    ).trim(),
...
function getVehicleWalkerArmCount(vehicleSystem = {}) {
  return Math.min(
    ...
    toNonNegativeWhole(
      vehicleSystem?.walker?.armCount ?? vehicleSystem?.walker?.arms?.count,
      2,
    ),
  );
}
...
function getVehicleWalkerArmTrackerId(armNumber = 0) {
  ...
  return resolvedNumber > 0
    ? `${MYTHIC_VEHICLE_WALKER_ARM_TRACKER_PREFIX}${resolvedNumber}`
    : "";
}
...
function normalizeVehicleWalkerArmMap(armsById = {}, armCount = 2) {
  const source =
    armsById && typeof armsById === "object" && !Array.isArray(armsById)
      ? armsById
      : {};
  const normalized = Object.fromEntries(
    Object.entries(source)
      ...
      .map(([id, arm]) => [
        String(id).trim(),
        normalizeVehicleWalkerArmState(arm),
      ]),
  );
  const safeArmCount = Math.min(
    MYTHIC_VEHICLE_WALKER_MAX_ARM_COUNT,
    toNonNegativeWhole(armCount, 2),
  );
...
function normalizeVehicleWalkerMountLocation(value = "") {
  ...
  if (MYTHIC_VEHICLE_WALKER_LOCATION_KEYS.includes(rawLocation))
    return rawLocation;
...
function normalizeVehicleWalkerMeleeData(melee = {}) {
  const source =
    melee && typeof melee === "object" && !Array.isArray(melee) ? melee : {};
  const punchSource =
    source.punch &&
    typeof source.punch === "object" &&
    !Array.isArray(source.punch)
      ? source.punch
      : {};
  const stompSource =
    source.stomp &&
    typeof source.stomp === "object" &&
    !Array.isArray(source.stomp)
      ? source.stomp
      : {};
  const requestedPunchModifier = String(punchSource?.modifier ?? "strength")
    .trim()
    .toLowerCase();
  const normalizeDiceSize = (value) => {
    const requestedSize = toNonNegativeWhole(value, 10);
    return MYTHIC_VEHICLE_WALKER_MELEE_DICE_SIZES.includes(requestedSize)
      ? requestedSize
      : 10;
  };
  return {
    punch: {
      ...
      modifier: MYTHIC_VEHICLE_WALKER_PUNCH_MODIFIERS.includes(
        requestedPunchModifier,
      )
        ? requestedPunchModifier
        : "strength",
    },
...
function isVehicleWalkerMountLocationUnavailable(
  vehicleSystem = {},
  location = "",
) {
  ...
  const fixedLocationState =
    vehicleSystem?.walker?.locations?.[walkerLocation] ?? null;
  if (
    fixedLocationState &&
    (fixedLocationState.destroyed || fixedLocationState.disabled)
  )
    return true;
  ...
  const mobilityMax = getVehicleConfiguredBaseValue(
    vehicleSystem?.breakpoints?.mob ?? {},
  );
  const mobilityTracker =
    vehicleSystem?.overview?.breakpoints?.mobility?.byId?.[armTrackerId] ??
    null;
  const trackerCurrent = Number(mobilityTracker?.current ?? mobilityMax);
  return (
    mobilityMax > 0 && Number.isFinite(trackerCurrent) && trackerCurrent <= 0
  );
}
...
function getVehicleWalkerDerivedStats(vehicleSystem = {}) {
  ...
  const pilotEvasionSkill = toNonNegativeWhole(
    vehicleSystem?.walker?.derived?.evasion?.pilotEvasionSkill,
    0,
  );
  ...
  const weightTonnes = Math.max(
    0,
    Number(vehicleSystem?.dimensions?.weight ?? 0) || 0,
  );
  const configuredJump = toNonNegativeNumber(
    vehicleSystem?.movement?.walker?.jump,
    0,
  );
  const configuredLeap = toNonNegativeNumber(
    vehicleSystem?.movement?.walker?.leap,
    0,
  );
  ...
  const calculatedLeap = roundToOneDecimal(
    Math.max(0, Math.max(strengthModifier / 2, agilityModifier / 2)),
  );
...
function getVehicleOverviewWeaponTrackerIds(vehicleSystem = {}) {
  const emplacements = Array.isArray(vehicleSystem?.weaponEmplacements)
...
function getVehicleOverviewOpticsTrackerIds(vehicleSystem = {}) {
  ...
  const ids = [];
  const operatorCount = toNonNegativeWhole(
    vehicleSystem?.crew?.capacity?.operators,
    0,
  );
  ...
  for (let index = 0; index < emplacements.length; index += 1) {
    ...
    const controllerRole = String(emplacement?.controllerRole ?? "")
      .trim()
      .toLowerCase();
...
function getVehicleOverviewMobilityTrackerIds(vehicleSystem = {}) {
  const propulsionType = normalizeVehiclePropulsionType(
    vehicleSystem?.propulsion?.type,
  );
  ...
  if (propulsionType === "legs") {
    ...
    return [
      "mobility-1",
      "mobility-2",
      ...Array.from({ length: armCount }, (_entry, index) =>
        getVehicleWalkerArmTrackerId(index + 1),
      ),
    ];
  }
  ...
  const configuredCount = Math.max(
    0,
    Number.parseInt(String(vehicleSystem?.propulsion?.max ?? ""), 10) ||
      toNonNegativeWhole(vehicleSystem?.propulsion?.value, 0),
  );

  return Array.from(
    { length: configuredCount },
    (_entry, index) => `mobility-${index + 1}`,
  );
}
...
function normalizeVehicleOverviewCustomBreakpoints(entries = []) {
  return (Array.isArray(entries) ? entries : [])
    .map((entry, index) => {
      ...
      const currentValue = clampWholeInRange(entry?.current, currentMax, {
        min: 0,
      });
      return {
        id:
          String(entry?.id ?? `custom-breakpoint-${index + 1}`).trim() ||
          `custom-breakpoint-${index + 1}`,
...
function buildVehicleTokenHudCrewShortSlotLabel(
  roleLabel = "Seat",
  slotNumber = 1,
) {
...
function getVehicleTokenHudEmplacementSlotCode(
  emplacement = {},
  fallbackIndex = 0,
) {
  const controllerRole = String(emplacement?.controllerRole ?? "")
...
function getVehicleTokenHudWalkerArmLabel(
  armNumber = 0,
  { short = false } = {},
) {
  const resolvedNumber = toNonNegativeWhole(armNumber, 0);
...
function getVehicleTokenHudMobilityComponentLabel(
  propulsionType = "wheels",
  plural = false,
) {
  const type = normalizeVehiclePropulsionType(propulsionType);
...
function buildVehicleTokenHudRow({
  ...
  const resolvedMin = Math.round(
    Number.isFinite(Number(min)) ? Number(min) : 0,
  );
  ...
  const resolvedFullLabel =
    String(fullLabel ?? label ?? id ?? resolvedLabel).trim() || resolvedLabel;
...
export function buildVehicleTokenHudBreakpointSections(vehicleSystem = {}) {
  const source =
    vehicleSystem &&
    typeof vehicleSystem === "object" &&
    !Array.isArray(vehicleSystem)
      ? vehicleSystem
  ...
  const overview =
    source?.overview && typeof source.overview === "object"
      ? source.overview
      : {};
  const emplacements = Array.isArray(source?.weaponEmplacements)
    ? source.weaponEmplacements
    : [];
  const operatorRows = Array.isArray(source?.crew?.operators)
    ? source.crew.operators
    : [];

  const engineMax = getVehicleConfiguredBaseValue(
    source?.breakpoints?.eng ?? {},
  );
  const hullMax = getVehicleConfiguredBaseValue(
    source?.breakpoints?.hull ?? {},
  );
  const weaponMax = getVehicleConfiguredBaseValue(
    source?.breakpoints?.wep ?? {},
  );
  const opticsMax = getVehicleConfiguredBaseValue(
    source?.breakpoints?.op ?? {},
  );
  const mobilityMax = getVehicleConfiguredBaseValue(
    source?.breakpoints?.mob ?? {},
  );
  ...
  const weaponRows = emplacements.map((emplacement, index) => {
    const emplacementId =
      String(emplacement?.id ?? `weapon-${index + 1}`).trim() ||
      `weapon-${index + 1}`;
  ...
  const operatorCount = Math.max(
    operatorRows.length,
    toNonNegativeWhole(source?.crew?.capacity?.operators, 0),
  );
  const opticsRows = opticsEnabled
    ? [
        ...Array.from({ length: operatorCount }, (_entry, index) => {
          ...
          const slotCode =
            String(
              operatorRows[index]?.slotCode ??
                buildVehicleTokenHudCrewShortSlotLabel("Operator", index + 1),
            ).trim() ||
            buildVehicleTokenHudCrewShortSlotLabel("Operator", index + 1);
          const seatLabel =
            String(
              operatorRows[index]?.seatLabel ?? `Operator ${index + 1}`,
            ).trim() || `Operator ${index + 1}`;
          return buildVehicleTokenHudRow({
            id: trackerId,
            label: slotCode,
        ...
        ...emplacements.reduce((rows, emplacement, index) => {
          if (
            String(emplacement?.controllerRole ?? "")
              .trim()
          ...
          const weaponId =
            String(emplacement?.id ?? `weapon-${index + 1}`).trim() ||
            `weapon-${index + 1}`;
          const slotCode = getVehicleTokenHudEmplacementSlotCode(
            emplacement,
            index,
          );
          ...
          rows.push(
            buildVehicleTokenHudRow({
              ...
              current:
                overview?.breakpoints?.optics?.byId?.[trackerId]?.current,
              max: opticsMax,
              inputName: `system.overview.breakpoints.optics.byId.${trackerId}.current`,
              min: 0,
            }),
  ...
  const propulsionType = normalizeVehiclePropulsionType(
    source?.propulsion?.type,
  );
  const configuredMobilityCount = Math.max(
    0,
    Number.parseInt(String(source?.propulsion?.max ?? ""), 10) ||
      toNonNegativeWhole(source?.propulsion?.value, 0),
  );
    ...
    propulsionType === "legs"
      ? [
          buildVehicleTokenHudRow({
            ...
            fullLabel: "Left Leg",
            current:
              overview?.breakpoints?.mobility?.byId?.["mobility-1"]?.current,
            max: mobilityMax,
            inputName:
              "system.overview.breakpoints.mobility.byId.mobility-1.current",
            min: 0,
          }),
          buildVehicleTokenHudRow({
            id: "mobility-2",
            ...
            fullLabel: "Right Leg",
            current:
              overview?.breakpoints?.mobility?.byId?.["mobility-2"]?.current,
            max: mobilityMax,
            inputName:
              "system.overview.breakpoints.mobility.byId.mobility-2.current",
            min: 0,
          }),
          ...Array.from({ length: walkerArmCount }, (_entry, index) => {
            ...
            return buildVehicleTokenHudRow({
              id: armId,
              label: getVehicleTokenHudWalkerArmLabel(armNumber, {
                short: armNumber <= 2,
              }),
      ...
      : Array.from({ length: configuredMobilityCount }, (_entry, index) => {
          ...
          const componentLabel = getVehicleTokenHudMobilityComponentLabel(
            propulsionType,
            false,
          );
          return buildVehicleTokenHudRow({
            ...
            current:
              overview?.breakpoints?.mobility?.byId?.[trackerId]?.current,
            max: mobilityMax,
  ...
  const customRows = (
    Array.isArray(overview?.breakpoints?.custom)
      ? overview.breakpoints.custom
      : []
  ).map((entry, index) => {
    const fallbackLabel = `C${index + 1}`;
    const fullLabel =
      String(entry?.label ?? "").trim() || `Custom ${index + 1}`;
    return buildVehicleTokenHudRow({
      id:
        String(entry?.id ?? `custom-breakpoint-${index + 1}`).trim() ||
        `custom-breakpoint-${index + 1}`,
...
function normalizeVehicleOverviewState(overview = {}, vehicleSystem = {}) {
  ...
  const sourceBreakpoints =
    source.breakpoints && typeof source.breakpoints === "object"
      ? source.breakpoints
      : {};
  const sourceArmor =
    source.armor && typeof source.armor === "object" ? source.armor : {};
  const sourceShields =
    source.shields && typeof source.shields === "object" ? source.shields : {};
  const mergedOverview = foundry.utils.mergeObject(
  ...
  mergedOverview.ui.breakpointsExpanded =
    mergedOverview.ui?.breakpointsExpanded !== false;
  for (const key of ["engineHull", "weapons", "optics", "mobility", "custom"]) {
    const fallback = key === "engineHull" || key === "custom";
    mergedOverview.ui.sections[key] =
      mergedOverview.ui?.sections?.[key] !== undefined
        ? Boolean(mergedOverview.ui.sections[key])
        : fallback;
  }

  const engineMax = getVehicleConfiguredBaseValue(
    vehicleSystem?.breakpoints?.eng ?? {},
  );
  const hullMax = getVehicleConfiguredBaseValue(
    vehicleSystem?.breakpoints?.hull ?? {},
  );
  const weaponsMax = getVehicleConfiguredBaseValue(
    vehicleSystem?.breakpoints?.wep ?? {},
  );
  const opticsMax = getVehicleConfiguredBaseValue(
    vehicleSystem?.breakpoints?.op ?? {},
  );
  const mobilityMax = getVehicleConfiguredBaseValue(
    vehicleSystem?.breakpoints?.mob ?? {},
  );
  ...
  const hasEngineCurrent =
    sourceBreakpoints?.engine?.current !== undefined &&
    sourceBreakpoints?.engine?.current !== null &&
    sourceBreakpoints?.engine?.current !== "";
  const hasHullCurrent =
    sourceBreakpoints?.hull?.current !== undefined &&
    sourceBreakpoints?.hull?.current !== null &&
    sourceBreakpoints?.hull?.current !== "";
  mergedOverview.breakpoints.engine.current = clampWholeInRange(
    hasEngineCurrent ? sourceBreakpoints.engine.current : engineMax,
    engineMax,
    {
      min: 0,
      max: engineMax,
  ...
  mergedOverview.breakpoints.hull.current = clampWholeInRange(
    hasHullCurrent ? sourceBreakpoints.hull.current : hullMax,
    hullMax,
    {
      min: MYTHIC_VEHICLE_HULL_NEGATIVE_FLOOR,
      max: hullMax,
  ...
  const normalizeTrackerMap = (
    ...
    const normalized = preserveExtra
      ? Object.fromEntries(
          Object.entries(mapSource).map(([id, tracker]) => {
            const sourceTracker =
              tracker && typeof tracker === "object" ? tracker : {};
            const hasCurrent =
              sourceTracker?.current !== undefined &&
              sourceTracker?.current !== null &&
              sourceTracker?.current !== "";
            return [
              id,
              {
                current: clampWholeInRange(
                  hasCurrent ? sourceTracker.current : maxValue,
                  maxValue,
                  { min: 0, max: maxValue },
                ),
              },
    ...
    for (const id of ids) {
      const tracker =
        mapSource[id] && typeof mapSource[id] === "object" ? mapSource[id] : {};
      const hasCurrent =
      ...
      normalized[id] = {
        current: clampWholeInRange(
          hasCurrent ? tracker.current : maxValue,
          maxValue,
          { min: 0, max: maxValue },
        ),
      };
  ...
  mergedOverview.breakpoints.custom = normalizeVehicleOverviewCustomBreakpoints(
    mergedOverview.breakpoints?.custom,
  );

  mergedOverview.armor ??= {};
  for (const key of MYTHIC_VEHICLE_ARMOR_LOCATION_KEYS) {
    const baseArmor = getVehicleConfiguredBaseValue(
      vehicleSystem?.armor?.[key] ?? {},
    );
    mergedOverview.armor[key] ??= {};
    const rawCurrent = sourceArmor?.[key]?.current;
    const hasCurrent =
      rawCurrent !== undefined && rawCurrent !== null && rawCurrent !== "";
    mergedOverview.armor[key].current = clampWholeInRange(
      hasCurrent ? rawCurrent : baseArmor,
      baseArmor,
      {
        min: 0,
        max: baseArmor,
  ...
  const shieldDelay = clampWholeInRange(vehicleSystem?.shields?.delay, 0, {
    min: 0,
  });
  const shieldRecharge = clampWholeInRange(
    vehicleSystem?.shields?.recharge,
    0,
    { min: 0 },
  );
  ...
  mergedOverview.shields.current = clampWholeInRange(
    hasInitializedShieldState ? sourceShields?.current : shieldMax,
    shieldMax,
    {
      min: 0,
      max: shieldMax,
...
export function normalizeCharacterSystemData(systemData) {
  ...
  const hadWoundsCurrent = foundry.utils.hasProperty(
    source,
  ...
  for (const key of MYTHIC_CHARACTERISTIC_KEYS) {
    ...
    merged.characteristics[key] = Number.isFinite(value)
      ? Math.max(0, value)
      : 0;
  }
  ...
  merged.mythic.outlierCharacteristicModifiers =
    getCharacterOutlierMythicCharacteristicModifiers(merged);
  merged.mythic.characteristics = getCharacterEffectiveMythicCharacteristics(
    merged,
    {
  ...
  const legacyCarryMultiplierRaw = Number(
    merged.mythic?.soldierTypeCarryMultiplier ?? 1,
  );
  const legacyCarryMultiplier = Number.isFinite(legacyCarryMultiplierRaw)
    ? Math.max(0, legacyCarryMultiplierRaw)
    : 1;
  const strCarryMultiplierRaw = Number(
    merged.mythic?.soldierTypeStrCarryMultiplier ?? legacyCarryMultiplier,
  );
  merged.mythic.soldierTypeStrCarryMultiplier = Number.isFinite(
    strCarryMultiplierRaw,
  )
    ? Math.max(0, strCarryMultiplierRaw)
    : 1;
  const touCarryMultiplierRaw = Number(
    merged.mythic?.soldierTypeTouCarryMultiplier ?? legacyCarryMultiplier,
  );
  merged.mythic.soldierTypeTouCarryMultiplier = Number.isFinite(
    touCarryMultiplierRaw,
  )
    ? Math.max(0, touCarryMultiplierRaw)
    : 1;
  const touWoundsMultiplierRaw = Number(
    merged.mythic?.soldierTypeTouWoundsMultiplier ?? 1,
  );
  merged.mythic.soldierTypeTouWoundsMultiplier = Number.isFinite(
    touWoundsMultiplierRaw,
  )
    ? Math.max(0, touWoundsMultiplierRaw)
    : 1;
  const soldierTypeLeapMultiplierRaw = Number(
    merged.mythic?.soldierTypeLeapMultiplier ?? 1,
  );
  merged.mythic.soldierTypeLeapMultiplier = Number.isFinite(
    soldierTypeLeapMultiplierRaw,
  )
    ? Math.max(0, soldierTypeLeapMultiplierRaw)
    : 1;
  const soldierTypeJumpMultiplierRaw = Number(
    merged.mythic?.soldierTypeJumpMultiplier ?? 1,
  );
  merged.mythic.soldierTypeJumpMultiplier = Number.isFinite(
    soldierTypeJumpMultiplierRaw,
  )
    ? Math.max(0, soldierTypeJumpMultiplierRaw)
    : 1;
  const soldierTypeLeapModifierRaw = Number(
    merged.mythic?.soldierTypeLeapModifier ?? 0,
  );
  merged.mythic.soldierTypeLeapModifier = Number.isFinite(
    soldierTypeLeapModifierRaw,
  )
    ? soldierTypeLeapModifierRaw
    : 0;
  const soldierTypeLeapAgiBonusRaw = Number(
    merged.mythic?.soldierTypeLeapAgiBonus ?? 0,
  );
  merged.mythic.soldierTypeLeapAgiBonus = Number.isFinite(
    soldierTypeLeapAgiBonusRaw,
  )
    ? soldierTypeLeapAgiBonusRaw
    : 0;
  const miscLeapModifierRaw = Number(merged.mythic?.miscLeapModifier ?? 0);
  merged.mythic.miscLeapModifier = Number.isFinite(miscLeapModifierRaw)
    ? miscLeapModifierRaw
    : 0;
  const miscCarryBonusRaw = Number(merged.mythic?.miscCarryBonus ?? 0);
  merged.mythic.miscCarryBonus = Number.isFinite(miscCarryBonusRaw)
    ? miscCarryBonusRaw
    : 0;
  const miscWoundsModifierRaw = Number(merged.mythic?.miscWoundsModifier ?? 0);
  merged.mythic.miscWoundsModifier = Number.isFinite(miscWoundsModifierRaw)
    ? miscWoundsModifierRaw
    : 0;
  ...
  const naturalArmorModifierRaw = Number(
    merged.mythic?.naturalArmorModifier ?? 0,
  );
  merged.mythic.naturalArmorModifier = Number.isFinite(naturalArmorModifierRaw)
    ? naturalArmorModifierRaw
    : 0;
  ...
  merged.mythic.fixedCarryWeight = Number.isFinite(fixedCarryWeightRaw)
    ? Math.max(0, fixedCarryWeightRaw)
    : 0;
  const chargeRunAgiBonusRaw = Number(
    merged.mythic?.soldierTypeChargeRunAgiBonus ?? 0,
  );
  merged.mythic.soldierTypeChargeRunAgiBonus = Number.isFinite(
    chargeRunAgiBonusRaw,
  )
    ? chargeRunAgiBonusRaw
    : 0;
  ...
  if (!hadWoundsCurrent) {
    ...
    merged.combat.wounds.current = Math.min(
      clampWhole(merged.combat.wounds.current),
      merged.combat.wounds.max,
    );
  }
  ...
  merged.combat.reactions.count = Math.max(
    0,
    Math.floor(Number(merged.combat.reactions?.count ?? 0)),
  );
  merged.combat.actionEconomy ??= {};
  merged.combat.actionEconomy.combatId = String(
    merged.combat.actionEconomy?.combatId ?? "",
  );
  merged.combat.actionEconomy.round = Math.max(
    0,
    Math.floor(Number(merged.combat.actionEconomy?.round ?? 0)),
  );
  merged.combat.actionEconomy.turn = Math.max(
    0,
    Math.floor(Number(merged.combat.actionEconomy?.turn ?? 0)),
  );
  merged.combat.actionEconomy.halfActionsSpent = Math.max(
    0,
    Math.floor(Number(merged.combat.actionEconomy?.halfActionsSpent ?? 0)),
  );
  merged.combat.actionEconomy.history = (
    Array.isArray(merged.combat.actionEconomy?.history)
      ? merged.combat.actionEconomy.history
      : []
  )
    ...
    .map((entry, index) => ({
      id:
        String(entry.id ?? `history-${index + 1}`).trim() ||
        `history-${index + 1}`,
  ...
  merged.combat.autoFireTracker.combatId = String(
    merged.combat.autoFireTracker?.combatId ?? "",
  );
  merged.combat.autoFireTracker.round = Math.max(
    0,
    Math.floor(Number(merged.combat.autoFireTracker?.round ?? 0)),
  );
  merged.combat.autoFireTracker.weapons =
    merged.combat.autoFireTracker?.weapons &&
    typeof merged.combat.autoFireTracker.weapons === "object" &&
    !Array.isArray(merged.combat.autoFireTracker.weapons)
      ? foundry.utils.deepClone(merged.combat.autoFireTracker.weapons)
      : {};
  merged.combat.targetSwitch ??= {};
  merged.combat.targetSwitch.combatId = String(
    merged.combat.targetSwitch?.combatId ?? "",
  );
  merged.combat.targetSwitch.round = Math.max(
    0,
    Math.floor(Number(merged.combat.targetSwitch?.round ?? 0)),
  );
  merged.combat.targetSwitch.lastTargetId = String(
    merged.combat.targetSwitch?.lastTargetId ?? "",
  );
  merged.combat.targetSwitch.switchCount = Math.max(
    0,
    Math.floor(Number(merged.combat.targetSwitch?.switchCount ?? 0)),
  );

  const gravRaw = Number(merged.gravity ?? 1.0);
  merged.gravity = Number.isFinite(gravRaw)
    ? Math.max(0, Math.min(4, Math.round(gravRaw * 10) / 10))
    : 1.0;
  merged.perceptiveRange ??= {};
  merged.perceptiveRange.lightingCondition = normalizePerceptiveLighting(
    merged.perceptiveRange?.lightingCondition ?? "normal",
  );

  merged.equipment ??= {};
  merged.equipment.credits = toNonNegativeWhole(merged.equipment.credits, 0);
  merged.equipment.carriedWeight = toNonNegativeWhole(
    merged.equipment.carriedWeight,
    0,
  );
  for (const key of [
    "primaryWeapon",
  ...
  merged.equipment.activePackSelection.value = String(
    merged.equipment?.activePackSelection?.value ?? "",
  ).trim();
  merged.equipment.activePackSelection.group = String(
    merged.equipment?.activePackSelection?.group ?? "",
  ).trim();
  merged.equipment.activePackSelection.name = String(
    merged.equipment?.activePackSelection?.name ?? "",
  ).trim();
  merged.equipment.activePackSelection.description = String(
    merged.equipment?.activePackSelection?.description ?? "",
  ).trim();
  merged.equipment.activePackSelection.items = normalizeStringList(
    Array.isArray(merged.equipment?.activePackSelection?.items)
      ? merged.equipment.activePackSelection.items
      : [],
  );
  merged.equipment.activePackSelection.packKey = String(
    merged.equipment?.activePackSelection?.packKey ?? "",
  ).trim();
  merged.equipment.activePackSelection.source = String(
    merged.equipment?.activePackSelection?.source ?? "",
  ).trim();
  merged.equipment.activePackSelection.appliedAt = String(
    merged.equipment?.activePackSelection?.appliedAt ?? "",
  ).trim();
  const rawPackGrants = Array.isArray(
  ...
  const rawAmmoPools =
    ...
    typeof merged.equipment.ammoPools === "object"
      ? merged.equipment.ammoPools
      : {};
  ...
  for (const [rawKey, rawPool] of Object.entries(rawAmmoPools)) {
    ...
    const hasPurchasedCount = Object.prototype.hasOwnProperty.call(
      pool,
      "purchasedCount",
    );
    ...
    const unitWeightOverrideKg =
      Number.isFinite(rawUnitWeightOverrideKg) && rawUnitWeightOverrideKg > 0
  ...
  const rawBallisticContainers =
    merged.equipment?.ballisticContainers &&
    typeof merged.equipment.ballisticContainers === "object"
      ? merged.equipment.ballisticContainers
      : {};
  const normalizedBallisticContainers = {};
  for (const [rawGroupKey, rawContainers] of Object.entries(
    rawBallisticContainers,
  )) {
    ...
    const containers = sourceContainers
      .map((entry) => {
        ...
        const current =
          capacity > 0 ? Math.min(capacity, currentRaw) : currentRaw;
        const typeRaw = String(container.type ?? "magazine")
          .trim()
          .toLowerCase();
        ...
        return {
          ...
          type,
          label:
            String(container.label ?? "").trim() ||
            (type === "belt" ? "Belt" : "Magazine"),
          capacity,
          current,
          ...
          compatibilitySignature:
            String(container.compatibilitySignature ?? "").trim() || groupKey,
          weightKg,
        };
      })
      .filter((entry) => entry.id);
  ...
  const rawIndependentAmmo =
    merged.equipment?.independentAmmo &&
    typeof merged.equipment.independentAmmo === "object"
      ? merged.equipment.independentAmmo
      : {};
  ...
  for (const [uuid, entry] of Object.entries(rawIndependentAmmo)) {
    ...
    const ammoImg =
      String(ammoEntry?.ammoImg ?? "").trim() || "icons/svg/item-bag.svg";
    ...
    } else if (quantity === 0 && MYTHIC_NORMALIZATION_DEBUG) {
      console.debug(
        "[NORMALIZATION] Filtering out ammo with 0 quantity:",
        ammoName,
      );
    }
  }
  const normalizedKeys = Object.keys(normalizedIndependentAmmo);
  if (rawKeys.length !== normalizedKeys.length && MYTHIC_NORMALIZATION_DEBUG) {
    console.debug(
      `[NORMALIZATION] Independent ammo: ${rawKeys.length} raw entries -> ${normalizedKeys.length} normalized entries`,
    );
  }
  merged.equipment.independentAmmo = normalizedIndependentAmmo;

  const rawEnergyCells =
    merged.equipment?.energyCells &&
    typeof merged.equipment.energyCells === "object"
      ? merged.equipment.energyCells
      : {};
    ...
    const cells = sourceCells
      .map((entry) => {
        ...
        const current = Math.min(
          capacity || toNonNegativeWhole(cell.current, 0),
          toNonNegativeWhole(cell.current, 0),
        );
        const ammoMode = String(cell.ammoMode ?? "")
          .trim()
          .toLowerCase();
        ...
        const sourceWeaponType = String(cell.sourceWeaponType ?? "")
          .trim()
          .toLowerCase();
        const sourceTraining = String(cell.sourceTraining ?? "")
          .trim()
          .toLowerCase();
        const compatibilitySignature = String(
        ...
        return {
          ...
          label:
            ...
            getEnergyCellLabel(ammoMode, batteryType),
          sourceWeaponName,
          sourceWeaponType,
          sourceTraining,
          compatibilitySignature,
  ...
  const rawWeaponState =
    merged.equipment?.weaponState &&
    typeof merged.equipment.weaponState === "object"
      ? merged.equipment.weaponState
      : {};
  ...
  for (const [rawId, rawState] of Object.entries(rawWeaponState)) {
    ...
    const specialAmmoPatternOptionIds = Array.isArray(
      state.specialAmmoPatternOptionIds,
    )
      ? state.specialAmmoPatternOptionIds.map((entry) =>
          String(entry ?? "").trim(),
        )
      : [];
    const specialAmmoPatternNextIndex =
      specialAmmoPatternOptionIds.length > 0
        ? toNonNegativeWhole(state.specialAmmoPatternNextIndex, 0) %
          specialAmmoPatternOptionIds.length
        : 0;
    normalizedWeaponState[itemId] = {
      ...
      magazineTrackingMode:
        String(state.magazineTrackingMode ?? "abstract")
          .trim()
      ...
      scopeMode:
        String(state.scopeMode ?? "none")
          .trim()
          .toLowerCase() || "none",
      fireMode: String(state.fireMode ?? "")
        .trim()
        .toLowerCase(),
  ...
  const legacyPrimary = String(
    merged.equipment?.equipped?.primaryWeaponId ?? "",
  ).trim();
  const legacySecondary = String(
    merged.equipment?.equipped?.secondaryWeaponId ?? "",
  ).trim();
  ...
  merged.equipment.equipped.armorId = String(
    merged.equipment?.equipped?.armorId ?? "",
  ).trim();
  merged.equipment.equipped.wieldedWeaponId = String(
    merged.equipment?.equipped?.wieldedWeaponId ?? "",
  ).trim();
  if (
    merged.equipment.equipped.wieldedWeaponId &&
    !merged.equipment.equipped.weaponIds.includes(
      merged.equipment.equipped.wieldedWeaponId,
    )
  ) {
    merged.equipment.equipped.wieldedWeaponId = "";
  ...
  merged.medical.gammaCompany =
    merged.medical?.gammaCompany &&
    typeof merged.medical.gammaCompany === "object"
      ? merged.medical.gammaCompany
      : {};
  merged.medical.gammaCompany.enabled = Boolean(
    merged.medical.gammaCompany?.enabled,
  );
  merged.medical.gammaCompany.smootherApplications = toNonNegativeWhole(
    merged.medical.gammaCompany?.smootherApplications,
    0,
  );
  merged.medical.gammaCompany.lastAppliedAt = String(
    merged.medical.gammaCompany?.lastAppliedAt ?? "",
  ).trim();
  merged.medical.activeEffects = (
    Array.isArray(merged.medical?.activeEffects)
      ? merged.medical.activeEffects
      : []
  )
  ...
  merged.advancements ??= {};
  merged.advancements.xpEarned = toNonNegativeWhole(
    merged.advancements.xpEarned,
    0,
  );
  merged.advancements.xpSpent = toNonNegativeWhole(
    merged.advancements.xpSpent,
    0,
  );
  for (const key of ["unlockedFeatures", "spendLog"]) {
    merged.advancements[key] = String(merged.advancements?.[key] ?? "");
  }
  merged.advancements.transactionNotes = String(
    merged.advancements?.transactionNotes ?? "",
  );
  merged.advancements.transactions = (
    Array.isArray(merged.advancements?.transactions)
      ? merged.advancements.transactions
      : []
  )
    .map((entry, index) => {
      ...
      return {
        id:
          String(entry?.id ?? `txn-${createdAt}-${index + 1}`).trim() ||
          `txn-${createdAt}-${index + 1}`,
        label,
  ...
  merged.advancements.purchases =
    merged.advancements?.purchases &&
    typeof merged.advancements.purchases === "object"
      ? merged.advancements.purchases
      : {};
  merged.advancements.purchases.woundUpgrades = toNonNegativeWhole(
    merged.advancements.purchases?.woundUpgrades,
    0,
  );

  const rawQueue =
    merged.advancements?.queue && typeof merged.advancements.queue === "object"
  ...
  merged.advancements.queue = {
    ...
    skillRanks: Object.fromEntries(
      Object.entries(
        rawQueue.skillRanks && typeof rawQueue.skillRanks === "object"
          ? rawQueue.skillRanks
          : {},
      )
    ...
    characteristicAdvancements: coerceNumberMap(
      rawQueue.characteristicAdvancements,
      0,
    ),
    characteristicOther: coerceNumberMap(rawQueue.characteristicOther, 0),
    languages: normalizeStringList(
      Array.isArray(rawQueue.languages) ? rawQueue.languages : [],
    ),
  };

  const rawOutliers =
    merged.advancements?.outliers &&
    typeof merged.advancements.outliers === "object"
      ? merged.advancements.outliers
      : {};
  const rawPurchases = Array.isArray(rawOutliers.purchases)
    ? rawOutliers.purchases
    : [];
  merged.advancements.outliers = {
  ...
  const rawCreationPath =
    merged.advancements?.creationPath &&
    typeof merged.advancements.creationPath === "object"
      ? merged.advancements.creationPath
      : {};
  const clampRoll = (value) =>
    Math.max(0, Math.min(999, toNonNegativeWhole(value, 0)));
  ...
  const lifestylesRaw = Array.isArray(rawCreationPath.lifestyles)
    ? rawCreationPath.lifestyles
    : [];
  const lifestyles = Array.from({ length: 3 }, (_, index) => {
    const entry =
      lifestylesRaw[index] && typeof lifestylesRaw[index] === "object"
        ? lifestylesRaw[index]
        : {};
    const mode =
      String(entry.mode ?? "manual")
        .trim()
        .toLowerCase() === "roll"
        ? "roll"
        : "manual";
  ...
  merged.advancements.creationPath = {
    ...
    upbringingSelections: normalizeChoiceSelections(
      rawCreationPath.upbringingSelections,
    ),
    environmentItemId: String(rawCreationPath.environmentItemId ?? "").trim(),
    environmentSelections: normalizeChoiceSelections(
      rawCreationPath.environmentSelections,
    ),
    lifestyles,
  };
  ...
  for (const key of [
    "enforceAbilityPrereqs",
    "showRollHints",
    "showWorkflowGuidance",
    "keepSidebarCollapsed",
    "preferTokenPreview",
  ]) {
    merged.settings.automation[key] = Boolean(
      merged.settings.automation?.[key],
    );
  }

  merged.biography = asRecord(merged.biography);
  merged.biography.languages = normalizeStringList(
    Array.isArray(merged.biography?.languages)
      ? merged.biography.languages
      : [],
  );
  merged.biography.physical = asRecord(merged.biography.physical);

  const rawHeightCm = Number(merged.biography.physical.heightCm);
  let normalizedHeightCm = Number.isFinite(rawHeightCm)
    ? Math.max(0, Math.round(rawHeightCm))
    : 0;
  if (normalizedHeightCm <= 0) {
    const parsedHeight = parseImperialHeightInput(
      merged.biography.physical.heightImperial ??
        merged.biography.physical.height ??
        "",
    );
    if (parsedHeight) {
      normalizedHeightCm = feetInchesToCentimeters(
        parsedHeight.feet,
        parsedHeight.inches,
      );
    }
  ...
  let normalizedWeightKg = Number.isFinite(rawWeightKg)
    ? Math.max(0, Math.round(rawWeightKg * 10) / 10)
    : 0;
  ...
  merged.biography.physical.heightImperial =
    normalizedHeightCm > 0 ? formatFeetInches(normalizedHeightCm) : "";
  merged.biography.physical.weightKg = normalizedWeightKg;
  merged.biography.physical.weightLbs =
    normalizedWeightKg > 0 ? kilogramsToPounds(normalizedWeightKg) : 0;
  ...
  merged.biography.physical.weight =
  ...
  if (normalizedHeightCm > 0) {
    ...
    merged.header.buildSize = getCanonicalSizeCategoryLabel(
      merged.header.buildSize,
    );
  }
  ...
  merged.specialization =
    merged.specialization && typeof merged.specialization === "object"
      ? merged.specialization
      : {};
  merged.specialization.selectedKey = String(
    merged.specialization.selectedKey ?? "",
  ).trim();
  ...
  merged.specialization.limitedApprovalChecked = Boolean(
    merged.specialization.limitedApprovalChecked,
  );

  // charBuilder normalization
  merged.charBuilder =
    merged.charBuilder && typeof merged.charBuilder === "object"
      ? merged.charBuilder
      : {};
  merged.charBuilder.managed = Boolean(merged.charBuilder.managed);
  merged.charBuilder.lowerTierUnlockEnabled = Boolean(
    merged.charBuilder.lowerTierUnlockEnabled,
  );
  ...
  for (const rowKey of [
    ...
    "creationPoints",
    "advancements",
    "purchasedAdvancements",
    "misc",
    "soldierTypeAdvancementsRow",
  ]) {
    merged.charBuilder[rowKey] =
      merged.charBuilder[rowKey] &&
      typeof merged.charBuilder[rowKey] === "object"
        ? merged.charBuilder[rowKey]
        : {};
    for (const statKey of MYTHIC_CHARACTERISTIC_KEYS) {
      ...
      if (
        rowKey === "advancements" ||
        rowKey === "purchasedAdvancements" ||
        rowKey === "soldierTypeAdvancementsRow"
      ) {
  ...
  merged.charBuilder.creationPoints.pool = Number.isFinite(rawPool)
    ? Math.max(1, Math.floor(rawPool))
    : 100;
  ...
  merged.ai.cognitivePatternGenerated = Boolean(
    merged.ai.cognitivePatternGenerated,
  );
  merged.ai.oniModel = String(merged.ai.oniModel ?? "").trim();
  merged.ai.oniLogicStructure = String(
    merged.ai.oniLogicStructure ?? "",
  ).trim();
  merged.ai.oniSerial = String(merged.ai.oniSerial ?? "").trim();

  merged.schemaVersion = coerceSchemaVersion(
...
function normalizeBestiaryArmorProfile(entry = {}, index = 0) {
  if (!entry || typeof entry !== "object" || Array.isArray(entry)) return null;
  const id =
    String(entry.id ?? `armor-${index + 1}`).trim() || `armor-${index + 1}`;
...
export function normalizeBestiarySystemData(systemData) {
  ...
  merged.bestiary.notes = Array.isArray(merged.bestiary?.notes)
    ? merged.bestiary.notes
    : [];
  ...
  const subtypeRaw = String(merged.bestiary?.subtype ?? "")
    .trim()
    .toLowerCase();
  ...
  merged.bestiary.advanceMythicStats = Boolean(
    merged.bestiary.advanceMythicStats,
  );
  const floodData =
    merged.bestiary?.flood && typeof merged.bestiary.flood === "object"
      ? merged.bestiary.flood
      : {};
  const formClassRaw = String(floodData.formClass ?? "")
    .trim()
    .toLowerCase();
  const keymindRoleRaw = String(floodData.keymindRole ?? "")
    .trim()
    .toLowerCase();
  ...
  merged.bestiary.flood = {
    ...
    keymindRole: allowedKeymindRoles.has(keymindRoleRaw)
      ? keymindRoleRaw
      : "none",
  };
  ...
  for (const key of MYTHIC_CHARACTERISTIC_KEYS) {
    ...
    merged.bestiary.baseCharacteristics[key] = Number.isFinite(baseValue)
      ? Math.max(0, Math.floor(baseValue))
      : 0;
    merged.bestiary.miscCharacteristics[key] = Number.isFinite(miscValue)
      ? Math.floor(miscValue)
      : 0;
  }

  for (const key of ["str", "tou", "agi"]) {
    const mythicValue = Number(merged.bestiary?.mythicBase?.[key] ?? 0);
    merged.bestiary.mythicBase[key] = Number.isFinite(mythicValue)
      ? Math.max(0, Math.floor(mythicValue))
      : 0;
  }
  merged.bestiary.mythicMisc ??= {};
  for (const key of ["str", "tou", "agi"]) {
    const miscValue = Number(merged.bestiary?.mythicMisc?.[key] ?? 0);
    merged.bestiary.mythicMisc[key] = Number.isFinite(miscValue)
      ? Math.trunc(miscValue)
      : 0;
  }

  const xpPayouts =
    merged.bestiary?.xpPayouts && typeof merged.bestiary.xpPayouts === "object"
  ...
  const woundsByRank =
    ...
    typeof merged.bestiary.woundsByRank === "object"
      ? merged.bestiary.woundsByRank
      : {};
  merged.bestiary.woundsByRank = {
  ...
  const luckByRank =
    ...
    typeof merged.bestiary.luckByRank === "object"
      ? merged.bestiary.luckByRank
      : {};
  ...
  merged.bestiary.heightRangeCm = normalizeRangeObject(
    merged.bestiary?.heightRangeCm,
    MYTHIC_DEFAULT_HEIGHT_RANGE_CM,
  );
  merged.bestiary.weightRangeKg = normalizeRangeObject(
    merged.bestiary?.weightRangeKg,
    MYTHIC_DEFAULT_WEIGHT_RANGE_KG,
  );

  const modifiers =
    merged.bestiary?.modifiers && typeof merged.bestiary.modifiers === "object"
      ? merged.bestiary.modifiers
      : {};
  merged.bestiary.modifiers = {
    jumpMultiplier: Number.isFinite(Number(modifiers.jumpMultiplier))
      ? Math.max(0, Number(modifiers.jumpMultiplier))
      : 1,
    leapAgiBonus: Number.isFinite(Number(modifiers.leapAgiBonus))
      ? Number(modifiers.leapAgiBonus)
      : 0,
    leapMultiplier: Number.isFinite(Number(modifiers.leapMultiplier))
      ? Math.max(0, Number(modifiers.leapMultiplier))
      : 1,
    runChargeAgiBonus: Number.isFinite(Number(modifiers.runChargeAgiBonus))
      ? Number(modifiers.runChargeAgiBonus)
      : 0,
    naturalArmor: Number.isFinite(Number(modifiers.naturalArmor))
      ? Number(modifiers.naturalArmor)
      : 0,
  };

  const equipmentListRaw = Array.isArray(merged.bestiary?.equipmentList)
    ? merged.bestiary.equipmentList
    : [];
  merged.bestiary.equipmentList = equipmentListRaw
    .map((entry, index) => {
      if (!entry || typeof entry !== "object") return null;
      const id =
        String(entry.id ?? `equipment-${index + 1}`).trim() ||
        `equipment-${index + 1}`;
      const name = String(entry.name ?? "").trim();
  ...
  merged.bestiary.equippedArmorId = String(
    merged.bestiary?.equippedArmorId ?? "",
  ).trim();
  const armorProfile =
    merged.bestiary?.armorProfile &&
    typeof merged.bestiary.armorProfile === "object" &&
    !Array.isArray(merged.bestiary.armorProfile)
      ? merged.bestiary.armorProfile
      : {};
  const armorSchema =
    armorProfile?.schema &&
    typeof armorProfile.schema === "object" &&
    !Array.isArray(armorProfile.schema)
      ? armorProfile.schema
      : {};
  const armorModifierDelta =
    armorProfile?.modifierDelta &&
    typeof armorProfile.modifierDelta === "object" &&
    !Array.isArray(armorProfile.modifierDelta)
      ? armorProfile.modifierDelta
      : {};
  const armorModifierDeltaMisc =
    armorModifierDelta?.misc &&
    typeof armorModifierDelta.misc === "object" &&
    !Array.isArray(armorModifierDelta.misc)
      ? armorModifierDelta.misc
      : {};
  const armorModifierDeltaMythic =
    armorModifierDelta?.mythic &&
    typeof armorModifierDelta.mythic === "object" &&
    !Array.isArray(armorModifierDelta.mythic)
      ? armorModifierDelta.mythic
      : {};
  merged.bestiary.armorProfile = {
    ...
    schema: {
      armorSystem:
        String(
          armorSchema.armorSystem ?? armorProfile.system ?? "none",
        ).trim() || "none",
      locations: Array.isArray(armorSchema.locations)
        ? armorSchema.locations
            .map((entry) => String(entry ?? "").trim())
            .filter(Boolean)
        : [],
      hasShields: Boolean(armorSchema.hasShields),
      unavailableLocations: Array.isArray(armorSchema.unavailableLocations)
        ? armorSchema.unavailableLocations
            .map((entry) => String(entry ?? "").trim())
            .filter(Boolean)
        : [],
    },
    notes: Array.isArray(armorProfile.notes)
      ? armorProfile.notes
          .map((entry) => String(entry ?? "").trim())
          .filter(Boolean)
      : [],
    metadata:
      armorProfile.metadata &&
      typeof armorProfile.metadata === "object" &&
      !Array.isArray(armorProfile.metadata)
  ...
  const weaponAmmoRaw =
    ...
    typeof merged.bestiary.weaponAmmo === "object" &&
    !Array.isArray(merged.bestiary.weaponAmmo)
      ? merged.bestiary.weaponAmmo
      : {};
  ...
  const rankModifier = merged.bestiary.singleDifficulty
    ? 0
    : getBestiaryCharacteristicBonus(rank);
  for (const key of MYTHIC_CHARACTERISTIC_KEYS) {
  ...
  const mythicBonus =
    !merged.bestiary.singleDifficulty && merged.bestiary.advanceMythicStats
  ...
  merged.biography.physical.heightRangeCm = foundry.utils.deepClone(
    merged.bestiary.heightRangeCm,
  );
  merged.biography.physical.weightRangeKg = foundry.utils.deepClone(
    merged.bestiary.weightRangeKg,
  );

  merged.mythic.soldierTypeTouWoundsMultiplier = 1;
  merged.mythic.soldierTypeJumpMultiplier =
    merged.bestiary.modifiers.jumpMultiplier;
  merged.mythic.soldierTypeLeapAgiBonus =
    merged.bestiary.modifiers.leapAgiBonus;
  merged.mythic.soldierTypeLeapMultiplier =
    merged.bestiary.modifiers.leapMultiplier;
  merged.mythic.soldierTypeChargeRunAgiBonus =
    merged.bestiary.modifiers.runChargeAgiBonus;
  merged.mythic.naturalArmorModifier = merged.bestiary.modifiers.naturalArmor;

  const woundsKey = `br${rank}`;
  const woundsMaxByRank = toNonNegativeWhole(
    merged.bestiary?.woundsByRank?.[woundsKey],
    0,
  );
  const currentWounds = toNonNegativeWhole(
    source.combat?.wounds?.current ?? mergedBase.combat?.wounds?.current,
    woundsMaxByRank,
  );
  ...
  const luckMaxByRank = toNonNegativeWhole(
    merged.bestiary?.luckByRank?.[woundsKey],
    0,
  );
  const currentLuck = toNonNegativeWhole(
    source.combat?.luck?.current ?? mergedBase.combat?.luck?.current,
    luckMaxByRank,
  );
  const finalLuckMax = merged.bestiary.subtype === "flood" ? 0 : luckMaxByRank;
  const finalLuckCurrent =
    merged.bestiary.subtype === "flood"
      ? 0
      : Math.min(currentLuck, luckMaxByRank);
...
export function normalizeVehicleSystemData(systemData) {
  try {
    if (
      systemData &&
      typeof systemData === "object" &&
      _normCache.vehicle.has(systemData)
    ) {
  ...
  merged.factionTraining =
    String(merged.factionTraining ?? "unsc").trim() || "unsc";
  ...
  for (const key of [
    ...
    merged.characteristics[key] = toNonNegativeWhole(
      merged.characteristics?.[key],
      0,
    );
  }
  ...
  for (const key of ["accelerate", "brake"]) {
    merged.movement[key] ??= {};
    merged.movement[key].value = toNonNegativeWhole(
      merged.movement[key]?.value,
      0,
    );
    merged.movement[key].max = toNonNegativeWhole(merged.movement[key]?.max, 0);
  }
  merged.movement.speed ??= {};
  merged.movement.speed.base = toNonNegativeWhole(
    merged.movement.speed?.base,
    0,
  );
  merged.movement.speed.value = toNonNegativeWhole(
    merged.movement.speed?.value,
    0,
  );
  merged.movement.speed.max = toNonNegativeWhole(merged.movement.speed?.max, 0);

  merged.movement.maneuver ??= {};
  merged.movement.maneuver.base = toNonNegativeWhole(
    merged.movement.maneuver?.base,
    0,
  );
  merged.movement.maneuver.total = toNonNegativeWhole(
    merged.movement.maneuver?.total,
    0,
  );
  merged.movement.maneuver.owner = String(
    merged.movement.maneuver?.owner ?? "",
  );
  ...
  for (const key of [
    ...
    merged.movement.walker[key] = toNonNegativeWhole(
      merged.movement.walker?.[key],
      0,
    );
  }
  for (const key of ["jump", "leap"]) {
    merged.movement.walker[key] = toNonNegativeNumber(
      merged.movement.walker?.[key],
      0,
    );
  }
  ...
  for (const key of ["wep", "mob", "eng", "op", "hull"]) {
    ...
    merged.breakpoints[key].value = toNonNegativeWhole(
      merged.breakpoints[key]?.value,
      0,
    );
    merged.breakpoints[key].max = toNonNegativeWhole(
      merged.breakpoints[key]?.max,
      0,
    );
  }
  merged.breakpoints.op.noOptics = coerceVehicleCheckboxBoolean(
    merged.breakpoints.op?.noOptics,
    false,
  );
  ...
  merged.breakpoints.hull.doom.countdown = normalizeVehicleDoomCountdownState(
    merged.breakpoints.hull?.doom?.countdown,
  );
  ...
  merged.doomed.persistent = normalizeVehicleDoomedPersistentLedger(
    merged?.doomed?.persistent,
  );
  ...
  const normalizeCrewRows = (rows = []) => {
    ...
    return sourceRows.map((entry, index) => {
      ...
      let assignmentType = normalizeCrewAssignmentType(
        entry?.assignmentType ?? entry?.referenceType ?? "",
      );
      const canonicalReference =
        requestedTokenUuid || requestedActorUuid || directId;

      if (!assignmentType && requestedTokenUuid) assignmentType = "token";
      if (!assignmentType && requestedActorUuid)
        assignmentType = "character-actor";
      if (
        !assignmentType &&
        /^Scene\.[^.]+\.Token\.[^.]+$/u.test(canonicalReference)
      )
        assignmentType = "token";
      if (!assignmentType && canonicalReference)
        assignmentType = "legacy-actor";
  ...
  merged.crew.capacity.operators = toNonNegativeWhole(
    merged.crew.capacity?.operators,
    0,
  );
  merged.crew.capacity.gunners = toNonNegativeWhole(
    merged.crew.capacity?.gunners,
    0,
  );
  merged.crew.capacity.passengers = toNonNegativeWhole(
    merged.crew.capacity?.passengers,
    0,
  );
  ...
  const ensureCrewArrayLength = (rows = [], capacity = 0) => {
    ...
    return result.map((entry, i) => ({
      ...
      position: String(entry?.position ?? "")
        .replace(/\s+/gu, " ")
        .trim(),
      assignmentType: normalizeCrewAssignmentType(
        entry?.assignmentType ?? entry?.referenceType ?? "",
      ),
  ...
  merged.crew.operators = ensureCrewArrayLength(
    merged.crew?.operators,
    merged.crew.capacity.operators,
  );
  merged.crew.gunners = ensureCrewArrayLength(
    merged.crew?.gunners,
    merged.crew.capacity.gunners,
  );
  merged.crew.complement = ensureCrewArrayLength(
    merged.crew?.complement,
    merged.crew.capacity.passengers,
  );
  ...
  for (const [key, fallback] of Object.entries(specialDefaults)) {
    ...
    merged.special[key].has = coerceVehicleCheckboxBoolean(
      merged.special[key]?.has,
      false,
    );
    if (Object.prototype.hasOwnProperty.call(fallback, "value")) {
      const fallbackValue =
        typeof fallback.value === "string"
          ...
          : toNonNegativeWhole(fallback.value, 0);
      if (
        typeof merged.special[key]?.value === "string" ||
        typeof fallbackValue === "string"
      ) {
        merged.special[key].value = String(
          merged.special[key]?.value ?? fallbackValue,
        );
      } else {
        merged.special[key].value = toNonNegativeWhole(
          merged.special[key]?.value,
  ...
  merged.propulsion ??= {};
  merged.propulsion.type = normalizeVehiclePropulsionType(
    merged.propulsion?.type,
  );
  ...
  const allowedPropulsionMax = getVehiclePropulsionMaxOptions(
    merged.propulsion.type,
  );
  ...
  merged.propulsion.state.multiplier = toNonNegativeNumber(
    merged.propulsion.state?.multiplier,
    1,
  );
  merged.propulsion.state.toHit =
    Number(merged.propulsion.state?.toHit ?? 0) || 0;
  merged.isWalker = merged.propulsion.type === "legs";
  merged.walker ??= {};
  merged.walker.armCount = getVehicleWalkerArmCount(merged);
  const requestedWalkerSizeCategory = getCanonicalSizeCategoryLabel(
    merged.walker?.sizeCategory ?? merged.walker?.buildSize ?? "",
  );
  merged.walker.sizeCategory = requestedWalkerSizeCategory || "Normal";
  merged.walker.reach = Math.max(
    0,
    Math.min(99, toNonNegativeWhole(merged.walker?.reach, 0)),
  );
  merged.walker.melee = normalizeVehicleWalkerMeleeData(merged.walker?.melee);
  merged.walker.arms ??= {};
  merged.walker.arms.byId = normalizeVehicleWalkerArmMap(
    merged.walker.arms?.byId,
    merged.walker.armCount,
  );
  ...
  for (const definition of MYTHIC_VEHICLE_WALKER_LOCATION_DEFINITIONS) {
    merged.walker.locations[definition.key] =
      normalizeVehicleWalkerLocationState(
        merged.walker.locations?.[definition.key],
        definition,
      );
  }
  for (const key of Object.keys(merged.walker.locations)) {
    ...
    const definition = MYTHIC_VEHICLE_WALKER_LOCATION_DEFINITIONS.find(
      (entry) => entry.key === key,
    );
    merged.walker.locations[key].breakpointType =
      definition?.breakpointType ?? merged.walker.locations[key].breakpointType;
  }
  merged.walker.derived = getVehicleWalkerDerivedStats(merged);

  merged.modifications ??= {};
  merged.modifications.mods = normalizeStringList(
    Array.isArray(merged.modifications?.mods) ? merged.modifications.mods : [],
  );
  ...
  merged.vehicle ??= {};
  merged.vehicle.ammoTrackingMode = ["none", "standard"].includes(
    String(merged.vehicle?.ammoTrackingMode ?? "")
      .trim()
      .toLowerCase(),
  )
  ...
  merged.vehicle.autoloader.enabled = coerceVehicleCheckboxBoolean(
    merged.vehicle.autoloader?.enabled,
    true,
  );
  ...
  const weaponEmplacementsArray = Array.isArray(rawWeaponEmplacements)
    ...
    : rawWeaponEmplacements && typeof rawWeaponEmplacements === "object"
      ? Object.values(rawWeaponEmplacements).filter(
          (entry) => entry && typeof entry === "object",
        )
      : [];
  merged.weaponEmplacements = weaponEmplacementsArray.map((entry, index) => {
    const requestedRole = String(entry?.controllerRole ?? "")
      .trim()
      .toLowerCase();
    const controllerRole = ["operator", "gunner", "passenger"].includes(
      requestedRole,
    )
      ? requestedRole
      : "";
    const linked = Boolean(entry?.linked);
    const requestedNeuralLinkOperatorSeatKey = String(
      entry?.neuralLinkOperatorSeatKey ?? "",
    ).trim();
    const neuralLinkOperatorSeatKey = /^operators:\d+$/u.test(
      requestedNeuralLinkOperatorSeatKey,
    )
      ? requestedNeuralLinkOperatorSeatKey
      : "";
    const sourceWeaponState =
      entry?.weaponState &&
      typeof entry.weaponState === "object" &&
      !Array.isArray(entry.weaponState)
        ? entry.weaponState
        : {};
    const sourceAmmo =
      entry?.ammo &&
      typeof entry.ammo === "object" &&
      !Array.isArray(entry.ammo)
        ? entry.ammo
        : {};
    ...
    const sourceMagazineArray = Array.isArray(rawSourceMagazines)
      ...
      : rawSourceMagazines && typeof rawSourceMagazines === "object"
        ? Object.values(rawSourceMagazines).filter(
            (entry) => entry && typeof entry === "object",
          )
        : [];
    const normalizedMagazines = sourceMagazineArray.map(
      (magazine, magazineIndex) => {
        const mag = magazine && typeof magazine === "object" ? magazine : {};
        const max = toNonNegativeWhole(mag.max, 0);
        return {
          id:
            String(mag.id ?? foundry.utils.randomID()).trim() ||
            foundry.utils.randomID(),
          type: String(mag.type ?? "Standard").trim() || "Standard",
          current: Math.max(
            0,
            Math.min(toNonNegativeWhole(mag.current, max), max),
          ),
          max,
    ...
    const requestedLocation = normalizeVehicleWalkerMountLocation(
      entry?.location,
    );
    return {
      id:
        String(entry?.id ?? `legacy-emplacement-${index + 1}`).trim() ||
        `legacy-emplacement-${index + 1}`,
      weaponItemId: String(entry?.weaponItemId ?? "").trim(),
      location: requestedLocation,
      unavailable: isVehicleWalkerMountLocationUnavailable(
        merged,
        requestedLocation,
      ),
      controllerRole,
      controllerIndex: controllerRole
        ? toNonNegativeWhole(
            entry?.controllerIndex ?? entry?.controllerPosition,
            0,
          )
        : 0,
      linked,
      linkedCount: linked
        ? Math.max(2, toNonNegativeWhole(entry?.linkedCount, 2))
        : 1,
      useNeuralLink: entry?.useNeuralLink === true,
      neuralLinkOperatorSeatKey,
      weaponState: {
        fireMode: String(sourceWeaponState.fireMode ?? "")
          .trim()
          .toLowerCase(),
        toHitModifier: Number.isFinite(Number(sourceWeaponState.toHitModifier))
          ? Math.round(Number(sourceWeaponState.toHitModifier))
          : 0,
        damageModifier: Number.isFinite(
          Number(sourceWeaponState.damageModifier),
        )
          ? Math.round(Number(sourceWeaponState.damageModifier))
          : 0,
        magazineCurrent: toNonNegativeWhole(
          sourceWeaponState.magazineCurrent,
          0,
        ),
        magazineTrackingMode:
          String(sourceWeaponState.magazineTrackingMode ?? "abstract")
            .trim()
            .toLowerCase() || "abstract",
        activeMagazineId: String(
          sourceWeaponState.activeMagazineId ?? "",
        ).trim(),
        activeEnergyCellId: String(
          sourceWeaponState.activeEnergyCellId ?? "",
        ).trim(),
        chamberRoundCount: toNonNegativeWhole(
          sourceWeaponState.chamberRoundCount,
          0,
        ),
        chargeLevel: toNonNegativeWhole(sourceWeaponState.chargeLevel, 0),
        rechargeRemaining: toNonNegativeWhole(
          sourceWeaponState.rechargeRemaining,
          0,
        ),
        variantIndex: toNonNegativeWhole(sourceWeaponState.variantIndex, 0),
        scopeMode:
          String(sourceWeaponState.scopeMode ?? "none")
            .trim()
            .toLowerCase() || "none",
      },
      ammo: {
        trackingMode: ["none", "standard"].includes(
          String(sourceAmmo.trackingMode ?? "")
            .trim()
            .toLowerCase(),
        )
  ...
  const hullMax = getVehicleConfiguredBaseValue(
    merged?.breakpoints?.hull ?? {},
  );
  const hullCurrent = Math.round(
    Number(merged?.overview?.breakpoints?.hull?.current ?? hullMax) || 0,
  );
  const baseArmorByLocation = Object.fromEntries(
    MYTHIC_VEHICLE_ARMOR_LOCATION_KEYS.map((key) => [
      key,
  ...
  merged.doomed = foundry.utils.mergeObject(
    merged.doomed,
    {
      ...
      doomedArmorPenaltyFlat: Math.max(
        0,
        Number(normalizedDoom?.doomedArmorPenaltyFlat ?? 0) || 0,
      ),
      doomedArmorCompromised: Boolean(normalizedDoom?.doomedArmorCompromised),
      doomedEffectiveArmorByLocation: foundry.utils.deepClone(
        normalizedDoom?.doomedEffectiveArmorByLocation ?? {},
      ),
      doomedVehicleImmobile: Boolean(normalizedDoom?.doomedVehicleImmobile),
      doomedEngineAimingDisabled: Boolean(
        normalizedDoom?.doomedEngineAimingDisabled,
      ),
      doomedOnFire: Boolean(normalizedDoom?.doomedOnFire),
      doomedFlameRating: Math.max(
        0,
        Number(normalizedDoom?.doomedFlameRating ?? 0) || 0,
      ),
      doomedOccupantsDamageMode: String(
        normalizedDoom?.doomedOccupantsDamageMode ?? "none",
      ),
      doomedDetonationState: String(
        normalizedDoom?.doomedDetonationState ?? "none",
      ),
      doomedDetonationRoundsRemaining: Math.max(
        0,
        Number(normalizedDoom?.doomedDetonationRoundsRemaining ?? 0) || 0,
      ),
      doomedDetonationSecondsRemaining: Math.max(
        0,
        Number(normalizedDoom?.doomedDetonationSecondsRemaining ?? 0) || 0,
      ),
      doomedBlastRadius: Math.max(
        0,
        Number(normalizedDoom?.doomedBlastRadius ?? 0) || 0,
      ),
      doomedKillRadius: Math.max(
        0,
        Number(normalizedDoom?.doomedKillRadius ?? 0) || 0,
      ),
      doomedImmediateDetonation: Boolean(
        normalizedDoom?.doomedImmediateDetonation,
      ),
      doomedHeavyPlatingLost: Boolean(normalizedDoom?.doomedHeavyPlatingLost),
      persistent: normalizeVehicleDoomedPersistentLedger(
        normalizedDoom?.persistent ?? {},
      ),
    },
    {
      inplace: false,
  ...
  merged.breakpoints.hull.doom = foundry.utils.mergeObject(
    merged.breakpoints.hull.doom,
    {
      ...
      doomedNegativeHullValue: Math.max(
        0,
        Number(normalizedDoom?.doomedNegativeHullValue ?? 0) || 0,
      ),
      doomedArmorPenaltyFlat: Math.max(
        0,
        Number(normalizedDoom?.doomedArmorPenaltyFlat ?? 0) || 0,
      ...
      doomedOccupantDamageMode: String(
        normalizedDoom?.doomedOccupantsDamageMode ?? "none",
      ),
      doomedDetonationState: String(
        normalizedDoom?.doomedDetonationState ?? "none",
      ...
      doomedHeavyPlatingLost: Boolean(normalizedDoom?.doomedHeavyPlatingLost),
      countdown: foundry.utils.deepClone(
        normalizedDoom?.countdown ?? MYTHIC_VEHICLE_DOOM_COUNTDOWN_DEFAULTS,
      ),
    },
    {
      inplace: false,
  ...
  if (Boolean(normalizedDoom?.doomedVehicleImmobile)) {
    if (
      merged?.overview?.breakpoints?.engine &&
      typeof merged.overview.breakpoints.engine === "object"
    ) {
      merged.overview.breakpoints.engine.current = 0;
    }
    const mobilityById =
      merged?.overview?.breakpoints?.mobility?.byId &&
      typeof merged.overview.breakpoints.mobility.byId === "object"
        ? merged.overview.breakpoints.mobility.byId
        : {};
  ...
  merged.perceptiveRange ??= {};
  merged.perceptiveRange.total = toNonNegativeWhole(
    merged.perceptiveRange?.total,
    0,
  );

  try {
    if (systemData && typeof systemData === "object")
      _normCache.vehicle.set(systemData, merged);
  } catch (_err) {
    // ignore
  }
...
export function normalizeSupportedItemSystemData(
  itemType,
  systemData,
  itemName = "",
) {
  if (itemType === "ability")
    return normalizeAbilitySystemData(systemData, itemName);
  if (itemType === "trait")
    return normalizeTraitSystemData(systemData, itemName);
  if (itemType === "education")
    return normalizeEducationSystemData(systemData, itemName);
  if (itemType === "armorVariant")
    return normalizeArmorVariantSystemData(systemData, itemName);
  if (itemType === "soldierType")
    return normalizeSoldierTypeSystemData(systemData, itemName);
  if (itemType === "gear") return normalizeGearSystemData(systemData, itemName);
  if (itemType === "upbringing")
    return normalizeUpbringingSystemData(systemData, itemName);
  if (itemType === "environment")
    return normalizeEnvironmentSystemData(systemData, itemName);
  if (itemType === "lifestyle")
    return normalizeLifestyleSystemData(systemData, itemName);
  return null;
}
...
export function normalizeSoldierTypeSystemData(systemData, itemName = "") {
  ...
  merged.heightRangeCm = normalizeRangeObject(
    merged.heightRangeCm,
    MYTHIC_DEFAULT_HEIGHT_RANGE_CM,
  );
  merged.weightRangeKg = normalizeRangeObject(
    merged.weightRangeKg,
    MYTHIC_DEFAULT_WEIGHT_RANGE_KG,
  );
  const canonicalBuildSize = getCanonicalSizeCategoryLabel(
    merged.header.buildSize,
  );
  if (canonicalBuildSize) {
    ...
    merged.header.buildSize =
      fallbackHeightCm > 0 ? getSizeCategoryFromHeightCm(fallbackHeightCm) : "";
  }
  ...
  merged.characteristicAdvancements =
    merged.characteristicAdvancements &&
    typeof merged.characteristicAdvancements === "object"
      ? merged.characteristicAdvancements
      : {};
  ...
  for (const key of MYTHIC_CHARACTERISTIC_KEYS) {
    const v = Math.max(
      0,
      Math.floor(Number(merged.characteristicAdvancements[key] ?? 0)),
    );
  ...
  const basePatches =
    merged.skills?.base && typeof merged.skills.base === "object"
      ? merged.skills.base
      : {};
  ...
  const customSource = Array.isArray(merged.skills?.custom)
    ? merged.skills.custom
    : [];
  merged.skills.custom = customSource.map((entry, index) => {
    const fallback = {
      ...
      characteristicOptions:
        Array.isArray(entry?.characteristicOptions) &&
        entry.characteristicOptions.length
  ...
  const rawSkillChoices = Array.isArray(merged.skillChoices)
    ? merged.skillChoices
    : [];
  ...
  const rawEducationChoices = Array.isArray(merged.educationChoices)
    ? merged.educationChoices
    : [];
  ...
  merged.advancementOptions = (
    Array.isArray(merged.advancementOptions) ? merged.advancementOptions : []
  )
    .map((entry, index) => normalizeSoldierTypeAdvancementOption(entry, index))
    .filter(Boolean);

  merged.training = Array.from(
    new Set(
  ...
  merged.customPromptMessages = (
    Array.isArray(merged.customPromptMessages)
      ? merged.customPromptMessages
      : []
  ).map((entry) => String(entry ?? ""));

  const branchTransitionSource =
    merged?.ruleFlags?.branchTransition &&
    typeof merged.ruleFlags.branchTransition === "object"
      ? merged.ruleFlags.branchTransition
      : {};
  merged.ruleFlags =
    merged.ruleFlags && typeof merged.ruleFlags === "object"
      ? merged.ruleFlags
      : {};
  merged.ruleFlags.airForceVehicleBenefit = Boolean(
    merged.ruleFlags?.airForceVehicleBenefit,
  );
  const carryMultipliersSource =
    merged.ruleFlags?.carryMultipliers &&
    typeof merged.ruleFlags.carryMultipliers === "object"
      ? merged.ruleFlags.carryMultipliers
      : {};
  const legacyCarryMultiplierRaw = Number(
    merged.ruleFlags?.carryMultiplier ?? 1,
  );
  const legacyCarryMultiplier = Number.isFinite(legacyCarryMultiplierRaw)
    ? Math.max(0, legacyCarryMultiplierRaw)
    : 1;
  ...
  const carryStrRaw = Number(
    carryMultipliersSource?.str ?? legacyCarryMultiplier,
  );
  const carryTouRaw = Number(
    carryMultipliersSource?.tou ?? legacyCarryMultiplier,
  );
  ...
  merged.ruleFlags.fixedCarryWeight = Number.isFinite(fixedCarryWeightRaw)
    ? Math.max(0, fixedCarryWeightRaw)
    : 0;
  merged.ruleFlags.chargeRunAgiBonus = Number.isFinite(chargeRunAgiBonusRaw)
    ? chargeRunAgiBonusRaw
    : 0;
  merged.ruleFlags.toughMultiplier = Number.isFinite(toughMultiplierRaw)
    ? Math.max(0, toughMultiplierRaw)
    : 1;
  merged.ruleFlags.leapMultiplier = Number.isFinite(leapMultiplierRaw)
    ? Math.max(0, leapMultiplierRaw)
    : 1;
  merged.ruleFlags.leapModifier = Number.isFinite(leapModifierRaw)
    ? leapModifierRaw
    : 0;
  merged.ruleFlags.branchTransition = {
    enabled: Boolean(branchTransitionSource?.enabled),
    advancementOnly: Boolean(branchTransitionSource?.advancementOnly),
    appliesInCharacterCreation:
      branchTransitionSource?.appliesInCharacterCreation === false
        ? false
        : true,
    transitionGroup: String(
      branchTransitionSource?.transitionGroup ?? "",
    ).trim(),
    fromSoldierTypes: normalizeStringList(
      Array.isArray(branchTransitionSource?.fromSoldierTypes)
        ? branchTransitionSource.fromSoldierTypes
        : [],
    ),
    notes: String(branchTransitionSource?.notes ?? "").trim(),
  };
  const orionAugmentationSource =
    merged?.ruleFlags?.orionAugmentation &&
    typeof merged.ruleFlags.orionAugmentation === "object"
      ? merged.ruleFlags.orionAugmentation
      : {};
  merged.ruleFlags.orionAugmentation = {
    ...
    appliesInCharacterCreation:
      orionAugmentationSource?.appliesInCharacterCreation === false
        ? false
        : true,
    transitionGroup: String(
      orionAugmentationSource?.transitionGroup ?? "",
    ).trim(),
    fromSoldierTypes: normalizeStringList(
      Array.isArray(orionAugmentationSource?.fromSoldierTypes)
        ? orionAugmentationSource.fromSoldierTypes
        : [],
    ),
    notes: String(orionAugmentationSource?.notes ?? "").trim(),
  };
  const oniSectionOneSource =
    merged?.ruleFlags?.oniSectionOne &&
    typeof merged.ruleFlags.oniSectionOne === "object"
      ? merged.ruleFlags.oniSectionOne
      : {};
  const oniRankSource =
    oniSectionOneSource?.rankScaffold &&
    typeof oniSectionOneSource.rankScaffold === "object"
      ? oniSectionOneSource.rankScaffold
      : {};
  const oniSupportSource =
    oniSectionOneSource?.supportScaffold &&
    typeof oniSectionOneSource.supportScaffold === "object"
      ? oniSectionOneSource.supportScaffold
      : {};
  const oniCostSource =
    oniSectionOneSource?.unscSupportCostScaffold &&
    typeof oniSectionOneSource.unscSupportCostScaffold === "object"
      ? oniSectionOneSource.unscSupportCostScaffold
      : {};
  merged.ruleFlags.oniSectionOne = {
    ...
    rankScaffold: {
      ...
      commandSpecializationAllowed: Boolean(
        oniRankSource?.commandSpecializationAllowed,
      ),
    ...
    supportScaffold: {
      ...
      grantAtCharacterCreation: Boolean(
        oniSupportSource?.grantAtCharacterCreation,
      ),
    ...
    unscSupportCostScaffold: {
      ...
      infantryMultiplier: Math.max(
        0,
        Number(oniCostSource?.infantryMultiplier ?? 1) || 1,
      ),
      ordnanceMultiplier: Math.max(
        0,
        Number(oniCostSource?.ordnanceMultiplier ?? 1) || 1,
      ),
      notes: String(oniCostSource?.notes ?? "").trim(),
    },
  };
  const smartAiSource =
    merged?.ruleFlags?.smartAi && typeof merged.ruleFlags.smartAi === "object"
      ? merged.ruleFlags.smartAi
      : {};
  merged.ruleFlags.smartAi = {
    enabled: Boolean(smartAiSource?.enabled),
    coreIdentityLabel:
      String(smartAiSource?.coreIdentityLabel ?? "Cognitive Pattern").trim() ||
      "Cognitive Pattern",
    notes: String(smartAiSource?.notes ?? "").trim(),
  };
  const naturalArmorScaffoldSource =
    merged?.ruleFlags?.naturalArmorScaffold &&
    typeof merged.ruleFlags.naturalArmorScaffold === "object"
      ? merged.ruleFlags.naturalArmorScaffold
      : {};
  merged.ruleFlags.naturalArmorScaffold = {
    ...
    halvedWhenArmored:
      naturalArmorScaffoldSource?.halvedWhenArmored === false ? false : true,
    halvedOnHeadshot:
      naturalArmorScaffoldSource?.halvedOnHeadshot === false ? false : true,
    notes: String(naturalArmorScaffoldSource?.notes ?? "").trim(),
  };

  const requiredUpbringingSource =
    merged?.ruleFlags?.requiredUpbringing &&
    typeof merged.ruleFlags.requiredUpbringing === "object"
      ? merged.ruleFlags.requiredUpbringing
      : {};
  merged.ruleFlags.requiredUpbringing = {
    enabled: Boolean(requiredUpbringingSource?.enabled),
    upbringing: String(requiredUpbringingSource?.upbringing ?? "").trim(),
    removeOtherUpbringings: Boolean(
      requiredUpbringingSource?.removeOtherUpbringings,
    ),
    notes: String(requiredUpbringingSource?.notes ?? "").trim(),
  };
  const mjolnirArmorSelectionSource =
    merged?.ruleFlags?.mjolnirArmorSelection &&
    typeof merged.ruleFlags.mjolnirArmorSelection === "object"
      ? merged.ruleFlags.mjolnirArmorSelection
      : {};
  merged.ruleFlags.mjolnirArmorSelection = {
    enabled: Boolean(mjolnirArmorSelectionSource?.enabled),
  };
  const spartanCarryWeightSource =
    merged?.ruleFlags?.spartanCarryWeight &&
    typeof merged.ruleFlags.spartanCarryWeight === "object"
      ? merged.ruleFlags.spartanCarryWeight
      : {};
  merged.ruleFlags.spartanCarryWeight = {
    enabled: Boolean(spartanCarryWeightSource?.enabled),
  };
  const phenomeChoiceSource =
    merged?.ruleFlags?.phenomeChoice &&
    typeof merged.ruleFlags.phenomeChoice === "object"
      ? merged.ruleFlags.phenomeChoice
      : {};
  ...
  merged.ruleFlags.phenomeChoice = {
    ...
    prompt:
      String(
        phenomeChoiceSource?.prompt ?? "Choose a Lekgolo phenome culture.",
      ).trim() || "Choose a Lekgolo phenome culture.",
    defaultKey: String(phenomeChoiceSource?.defaultKey ?? "").trim(),
    choices: phenomeChoices,
  };
  const allowedUpbringingsSource =
    merged?.ruleFlags?.allowedUpbringings &&
    typeof merged.ruleFlags.allowedUpbringings === "object"
      ? merged.ruleFlags.allowedUpbringings
      : {};
  merged.ruleFlags.allowedUpbringings = {
    enabled: Boolean(allowedUpbringingsSource?.enabled),
    upbringings: normalizeStringList(
      Array.isArray(allowedUpbringingsSource?.upbringings)
        ? allowedUpbringingsSource.upbringings
        : [],
    ),
    removeOtherUpbringings: Boolean(
      allowedUpbringingsSource?.removeOtherUpbringings,
    ),
    notes: String(allowedUpbringingsSource?.notes ?? "").trim(),
  };
  const allowedEnvironmentsSource =
    merged?.ruleFlags?.allowedEnvironments &&
    typeof merged.ruleFlags.allowedEnvironments === "object"
      ? merged.ruleFlags.allowedEnvironments
      : {};
  merged.ruleFlags.allowedEnvironments = {
    enabled: Boolean(allowedEnvironmentsSource?.enabled),
    environments: normalizeStringList(
      Array.isArray(allowedEnvironmentsSource?.environments)
        ? allowedEnvironmentsSource.environments
        : [],
    ),
    removeOtherEnvironments: Boolean(
      allowedEnvironmentsSource?.removeOtherEnvironments,
    ),
    notes: String(allowedEnvironmentsSource?.notes ?? "").trim(),
  };
  const allowedLifestylesSource =
    merged?.ruleFlags?.allowedLifestyles &&
    typeof merged.ruleFlags.allowedLifestyles === "object"
      ? merged.ruleFlags.allowedLifestyles
      : {};
  merged.ruleFlags.allowedLifestyles = {
    enabled: Boolean(allowedLifestylesSource?.enabled),
    lifestyles: normalizeStringList(
      Array.isArray(allowedLifestylesSource?.lifestyles)
        ? allowedLifestylesSource.lifestyles
        : [],
    ),
    removeOtherLifestyles: Boolean(
      allowedLifestylesSource?.removeOtherLifestyles,
    ),
    notes: String(allowedLifestylesSource?.notes ?? "").trim(),
  };
  const gammaCompanyOptionSource =
    merged?.ruleFlags?.gammaCompanyOption &&
    typeof merged.ruleFlags.gammaCompanyOption === "object"
      ? merged.ruleFlags.gammaCompanyOption
      : {};
  merged.ruleFlags.gammaCompanyOption = {
    ...
    grantAbility:
      String(
        gammaCompanyOptionSource?.grantAbility ?? "Adrenaline Rush",
      ).trim() || "Adrenaline Rush",
  };
  const ordinanceReadySource =
    merged?.ruleFlags?.ordinanceReady &&
    typeof merged.ruleFlags.ordinanceReady === "object"
      ? merged.ruleFlags.ordinanceReady
      : {};
  merged.ruleFlags.ordinanceReady = {
    enabled: Boolean(ordinanceReadySource?.enabled),
    supportPointCost: toNonNegativeWhole(
      ordinanceReadySource?.supportPointCost,
      1,
    ),
    maxUsesPerEncounter: toNonNegativeWhole(
      ordinanceReadySource?.maxUsesPerEncounter,
      1,
    ),
    notes: String(ordinanceReadySource?.notes ?? "").trim(),
  };

  const rawEquipmentPacks = Array.isArray(merged.equipmentPacks)
    ? merged.equipmentPacks
    : [];
    ...
    merged.specPacks = [
      {
        ...
        options: merged.equipmentPacks.map((entry, index) =>
          normalizeSoldierTypeEquipmentPack(entry, index),
        ),
      },
    ];
  }
  ...
  merged.creation =
    merged.creation && typeof merged.creation === "object"
      ? merged.creation
      : { xpCost: 0 };
...
export function normalizeGearSystemData(systemData, itemName = "") {
  try {
    if (
      systemData &&
      typeof systemData === "object" &&
      _normCache.gear.has(systemData)
    ) {
  ...
  const resolveEquipmentTypeFromLegacy = (value = {}) => {
    const itemClass = String(value?.itemClass ?? "")
      .trim()
      .toLowerCase();
    const weaponClass = String(value?.weaponClass ?? "")
      .trim()
      .toLowerCase();
    const category = String(value?.category ?? "")
      .trim()
      .toLowerCase();
    const weaponType = String(value?.weaponType ?? "")
      .trim()
      .toLowerCase();
    ...
    if (itemClass === "weapon") {
      if (/(?:\bammo\b|\bammunition\b|\bmag(?:azine)?s?\b)/u.test(hint))
        return "ammunition";
    ...
    if (
      itemClass === "explosive" ||
      itemClass === "explosives" ||
      itemClass === "grenade" ||
      itemClass === "grenades"
    )
      return "explosives-and-grenades";
    if (
      itemClass === "weapon-modification" ||
      itemClass === "weapon_modification" ||
      itemClass === "weaponmod"
    )
      return "weapon-modification";
    if (
      itemClass === "armor-modification" ||
      itemClass === "armor_modification" ||
      itemClass === "armormod"
    )
      return "armor-modification";
    if (
      itemClass === "armor-permutation" ||
      itemClass === "armor_permutation" ||
      itemClass === "armorpermutation"
    )
      return "armor-modification";
    if (
      itemClass === "ammo-modification" ||
      itemClass === "ammo_modification" ||
      itemClass === "ammomod" ||
      itemClass === "ammo-mod"
    )
      return "ammo-modification";

    if (/\bammo\b|\bammunition\b/u.test(hint)) return "ammunition";
    if (/\bcontainer\b|\bcrate\b|\bcase\b|\bpack\b/u.test(hint))
      return "container";
    if (
      /\bgrenade\b|\bexplosive\b|\bmine\b|\bsatchel\b|\bdemolition\b|\bordinance\b|\bordnance\b/u.test(
        hint,
      )
    )
      return "explosives-and-grenades";
    if (/\bweapon\s*mod\b|\bweapon\s*modification\b/u.test(hint))
      return "weapon-modification";
    if (
      /\barmor\s*mod\b|\barmou?r\s*modification\b|\barmou?r\s*permutation(s)?\b/u.test(
        hint,
      )
    )
      return "armor-modification";
    if (/\bammo\s*mod\b|\bammunition\s*modification\b/u.test(hint))
      return "ammo-modification";

    return "general";
  };
  const applyLegacyClassFromEquipmentType = (
    value = {},
  ...
  const itemClass = String(merged.itemClass ?? "weapon")
    .trim()
    .toLowerCase();
  merged.itemClass = itemClass || "weapon";

  const weaponClass = String(merged.weaponClass ?? "ranged")
    .trim()
    .toLowerCase();
  merged.weaponClass = [
    "ranged",
  ...
  const requestedEquipmentType = String(merged.equipmentType ?? "")
    .trim()
    .toLowerCase();
  const canonicalEquipmentType = [
    "armor-permutation",
    "armor-permutations",
  ].includes(requestedEquipmentType)
    ? "armor-modification"
  ...
  if (merged.equipmentType === "explosives-and-grenades") {
    const rawAmmoMode = String(source.ammoMode ?? merged.ammoMode ?? "")
      .trim()
      .toLowerCase();
    if (
      !rawAmmoMode ||
      rawAmmoMode === "magazine" ||
      rawAmmoMode === "standard"
    ) {
  ...
  merged.faction = String(merged.faction ?? "").trim();
  const requestedArmorySelection = String(merged.armorySelection ?? "")
    .trim()
    .toUpperCase();
  merged.armorySelection = [
    ...
    "BANISHED",
    "FORERUNNER",
  ].includes(requestedArmorySelection)
  ...
  if (!merged.faction && merged.armorySelection) {
    ...
    const factionUpper = String(merged.faction ?? "")
      .trim()
      .toUpperCase();
    merged.armorySelection = [
      "UNSC",
  ...
  merged.source =
    String(merged.source ?? "mythic")
      .trim()
      .toLowerCase() || "mythic";
  merged.category = String(merged.category ?? "").trim();
  merged.isUniversal =
    merged.isUniversal === true ||
    String(merged.isUniversal ?? "")
      .trim()
      .toLowerCase() === "true";
  merged.training = String(merged.training ?? "")
    .trim()
    .toLowerCase();
  merged.baseToHitModifier = Number.isFinite(Number(merged.baseToHitModifier))
    ? Math.round(Number(merged.baseToHitModifier))
    : 0;
  ...
  const requestedSpecialAmmoCategory = String(
    merged.specialAmmoCategory ?? legacyAmmoType,
  ).trim();
  merged.specialAmmoCategory = requestedSpecialAmmoCategory || "Standard";
  if (Object.hasOwn(merged, "ammoType")) delete merged.ammoType;
  merged.ammoClass =
    String(merged.ammoClass ?? "ballistic")
      .trim()
      .toLowerCase() || "ballistic";
  const requestedAmmoFamily = String(merged.family ?? "").trim();
  merged.family = MYTHIC_SPECIAL_AMMO_FAMILY_SET.has(requestedAmmoFamily)
    ? requestedAmmoFamily
    : "";
  merged.caliberOrType = String(
    merged.caliberOrType ?? merged.ammoName ?? itemName,
  ).trim();
  ...
  merged.modifierCodes = normalizeStringList(
    (Array.isArray(merged.modifierCodes)
      ? merged.modifierCodes
      : parseFlexibleList(merged.modifierCodes)
    ).map((entry) =>
      String(entry ?? "")
        .trim()
        .toUpperCase()
  ...
  merged.modifierIds = normalizeStringList(
    Array.isArray(merged.modifierIds)
      ? merged.modifierIds
      : parseFlexibleList(merged.modifierIds),
  );
  merged.isSpecial =
    merged.isSpecial === true || merged.modifierCodes.length > 0;
  merged.displayLabel = String(merged.displayLabel ?? itemName ?? "").trim();
  merged.displaySymbol = String(merged.displaySymbol ?? "").trim();
  const requestedCostModel = String(merged.costModel ?? "per100")
    .trim()
    .toLowerCase();
  merged.costModel = [
    "per100",
    ...
    "unit",
    "per-quantity",
  ].includes(requestedCostModel)
    ? requestedCostModel
    : "per100";
  const rawCostPerRoundDerived = Number(merged.costPerRoundDerived ?? 0);
  merged.costPerRoundDerived = Number.isFinite(rawCostPerRoundDerived)
    ? Math.max(0, rawCostPerRoundDerived)
    : 0;
  merged.quantityOwned = toNonNegativeWhole(
    merged.quantityOwned ?? merged.quantity ?? 1,
    1,
  );
  merged.stackKey = String(merged.stackKey ?? "").trim();
  merged.effectSnapshot =
    merged.effectSnapshot &&
    typeof merged.effectSnapshot === "object" &&
    !Array.isArray(merged.effectSnapshot)
  ...
  if (merged.equipmentType === "ammunition") {
    const definition =
      ...
      typeof merged.ammoTypeDefinition === "object" &&
      !Array.isArray(merged.ammoTypeDefinition)
        ? merged.ammoTypeDefinition
        : {};
    const definitionWeight = Number(
      definition.weightPerRoundKg ??
        definition.unitWeightKg ??
        definition.weightKg,
    );
    const currentWeight = Number(
      merged.weightPerRoundKg ?? merged.weightKg ?? 0,
    );
    if (
      (!Number.isFinite(currentWeight) || currentWeight <= 0) &&
      Number.isFinite(definitionWeight) &&
      definitionWeight > 0
    ) {
    ...
    const definitionCost = Number(
      definition.costPer100 ?? definition.price?.amount ?? definition.cost,
    );
    const currentCost = Number(merged.costPer100 ?? merged.price?.amount ?? 0);
    if (
      (!Number.isFinite(currentCost) || currentCost <= 0) &&
      Number.isFinite(definitionCost) &&
      definitionCost > 0
    ) {
    ...
    const definitionCategory = String(
      definition.specialAmmoCategory ?? "",
    ).trim();
    if (
      definitionCategory &&
      (!merged.specialAmmoCategory || merged.specialAmmoCategory === "Standard")
    ) {
      merged.specialAmmoCategory = definitionCategory;
    }
  }
  const rawWeightPerRoundKg = Number(
    merged.weightPerRoundKg ?? merged.weightKg ?? 0,
  );
  merged.weightPerRoundKg = Number.isFinite(rawWeightPerRoundKg)
    ? Math.max(0, rawWeightPerRoundKg)
    : 0;
  merged.costPer100 = toNonNegativeWhole(
    merged.costPer100 ?? merged.price?.amount ?? 0,
    0,
  );
  merged.nicknames = normalizeStringList(
    Array.isArray(merged.nicknames)
      ? merged.nicknames
      : parseList(merged.nicknames),
  );
  merged.fireModes = normalizeStringList(
    Array.isArray(merged.fireModes)
      ? merged.fireModes
      : parseList(merged.fireModes),
  );
  merged.charge.damagePerLevel = toNonNegativeWhole(
    merged.charge?.damagePerLevel,
    0,
  );
  merged.charge.ammoPerLevel = toNonNegativeWhole(
    merged.charge?.ammoPerLevel,
    1,
  );
  ...
  const requestedDiceType = String(merged.damage?.diceType ?? "d10")
    .trim()
    .toLowerCase();
  ...
  if (
    merged.damage.diceCount <= 0 &&
    (merged.damage.baseRollD10 > 0 || merged.damage.baseRollD5 > 0)
  ) {
  ...
  const acceptedStrModifierModes = [
    "double-str-mod",
    "full-str-mod",
    "half-str-mod",
    "none",
  ...
  const requestedBaseDamageModifierMode = String(
    merged.damage?.baseDamageModifierMode ?? "full-str-mod",
  )
    .trim()
    .toLowerCase();
  merged.damage.baseDamageModifierMode = acceptedStrModifierModes.includes(
    requestedBaseDamageModifierMode,
  )
    ? requestedBaseDamageModifierMode
    : "full-str-mod";
  merged.damage.pierce = Number.isFinite(Number(merged.damage?.pierce))
    ? Number(merged.damage.pierce)
    : 0;
  const requestedPierceModifierMode = String(
    merged.damage?.pierceModifierMode ?? "full-str-mod",
  )
    .trim()
    .toLowerCase();
  merged.damage.pierceModifierMode = acceptedStrModifierModes.includes(
  ...
  merged.price.currency =
    String(merged.price?.currency ?? "cr")
      .trim()
      .toLowerCase() || "cr";
  merged.weightKg = Number.isFinite(Number(merged.weightKg))
    ? Math.max(0, Number(merged.weightKg))
    : 0;
  ...
  merged.weaponSpecialRuleKeys = normalizeStringList(
    parseFlexibleList(merged.weaponSpecialRuleKeys),
  );
  const rawWeaponSpecialRuleValues =
    merged.weaponSpecialRuleValues &&
    typeof merged.weaponSpecialRuleValues === "object" &&
    !Array.isArray(merged.weaponSpecialRuleValues)
      ? merged.weaponSpecialRuleValues
      : {};
  ...
  merged.weaponTagKeys = normalizeStringList(
    parseFlexibleList(merged.weaponTagKeys),
  );
  ...
  const rawBreakPoints =
    merged.breakPoints &&
    typeof merged.breakPoints === "object" &&
    !Array.isArray(merged.breakPoints)
      ? merged.breakPoints
      : {};
  merged.breakPoints = {
    current: Number.isFinite(Number(rawBreakPoints.current))
      ? Number(rawBreakPoints.current)
      : 0,
    max: Number.isFinite(Number(rawBreakPoints.max))
      ? Number(rawBreakPoints.max)
      : 0,
  };
  merged.armor = Number.isFinite(Number(merged.armor))
    ? Number(merged.armor)
    : 0;
  merged.providesHandheldEnergyShield = Boolean(
    merged.providesHandheldEnergyShield,
  );
  ...
  const advancedSource =
    merged.advanced &&
    typeof merged.advanced === "object" &&
    !Array.isArray(merged.advanced)
      ? merged.advanced
      : {};
  merged.advanced = {
  ...
  const ammoModeRaw = String(merged.ammoMode ?? "magazine")
    .trim()
    .toLowerCase();
  merged.ammoMode = [
    "magazine",
    ...
    "light-mass",
    "grenade",
  ].includes(ammoModeRaw)
    ? ammoModeRaw
    : "magazine";
  merged.batteryType = normalizeBatterySubtype(
    merged.batteryType,
    merged.ammoMode,
  );
  merged.singleLoading = Boolean(merged.singleLoading);
  merged.variantAttacks = Array.isArray(merged.variantAttacks)
    ? merged.variantAttacks
        .map((v) => {
          if (!v || typeof v !== "object") return null;
          const vDiceCount = toNonNegativeWhole(v.diceCount, 0);
          const vDiceType =
            String(v.diceType ?? "d10")
              .trim()
              .toLowerCase() === "d5"
              ? "d5"
              : "d10";
          const vBaseDmgMode = acceptedStrModifierModes.includes(
            String(v.baseDamageModifierMode ?? "")
              .trim()
              .toLowerCase(),
          )
            ? String(v.baseDamageModifierMode).trim().toLowerCase()
            : "full-str-mod";
          const vPierceMode = acceptedStrModifierModes.includes(
            String(v.pierceModifierMode ?? "")
              .trim()
              .toLowerCase(),
          )
          ...
          return {
            ...
            diceType: vDiceType,
            baseDamage: Number.isFinite(Number(v.baseDamage))
              ? Number(v.baseDamage)
              : 0,
  ...
  if (merged.equipmentType === "ranged-weapon") {
    const rawTraining = String(merged.training ?? "")
      .trim()
      .toLowerCase();
  ...
  if (merged.equipmentType === "melee-weapon") {
    const meleeWeaponTypeValues = new Set(
      MYTHIC_MELEE_WEAPON_TYPE_OPTIONS.map((entry) =>
        String(entry?.value ?? "")
          .trim()
          .toLowerCase(),
      ).filter(Boolean),
    ...
    const trainingHint = String(merged.training ?? "")
      .trim()
      .toLowerCase();
    merged.training = ["basic", "melee"].includes(trainingHint)
      ? trainingHint
      : "basic";
  ...
  const armorProfileRaw = String(merged.armorWeightProfile ?? "")
    .trim()
    .toLowerCase();
  ...
  if (explicitArmorPowerProfile) {
    ...
    ["standard", "semi-powered", "powered"].includes(armorProfileRaw)
  ) {
  ...
  if (!Object.hasOwn(source, "isPoweredArmor")) {
    merged.isPoweredArmor = ["powered", "semi-powered"].includes(
      merged.armorWeightProfile,
    );
  } else {
  ...
  const powerStateOverrideRaw = String(merged.powerStateOverride ?? "default")
    .trim()
    .toLowerCase();
  ...
  merged.powerStateOverride = merged.isCurrentlyUnpowered
    ? "forced-off"
    : "default";
  merged.armorSpecialRuleKeys = normalizeStringList(
    parseFlexibleList(merged.armorSpecialRuleKeys),
  );
  merged.armorAbilityKeys = normalizeStringList(
    parseFlexibleList(merged.armorAbilityKeys),
  );
  merged.powerArmorTraitKeys = normalizeStringList(
    parseFlexibleList(merged.powerArmorTraitKeys),
  );
  merged.quantity = toNonNegativeWhole(merged.quantity ?? 1, 1);
  merged.quantityOwned = merged.quantity;
  merged.builtInItemIds = normalizeStringList(
    parseFlexibleList(merged.builtInItemIds),
  );
  merged.powerArmorTraitIds = normalizeStringList(
    parseFlexibleList(merged.powerArmorTraitIds),
  );
  merged.photoReactivePanelsBonus = Math.max(
    0,
    Math.min(99, toNonNegativeWhole(merged.photoReactivePanelsBonus, 0)),
  );
  const rawCharacteristicMods =
    merged.characteristicMods && typeof merged.characteristicMods === "object"
      ? merged.characteristicMods
      : {};
  const rawBaseMods =
    rawCharacteristicMods.base && typeof rawCharacteristicMods.base === "object"
      ? rawCharacteristicMods.base
      : {};
  const rawMythicMods =
    rawCharacteristicMods.mythic &&
    typeof rawCharacteristicMods.mythic === "object"
      ? rawCharacteristicMods.mythic
      : {};
  merged.characteristicMods = {
    base: Object.fromEntries(
      MYTHIC_CHARACTERISTIC_KEYS.map((key) => {
        const raw = Number(rawBaseMods?.[key] ?? 0);
        return [key, Number.isFinite(raw) ? raw : 0];
      }),
    ),
    mythic: Object.fromEntries(
      ["str", "tou", "agi"].map((key) => {
  ...
  const rawPierceReductions = Array.isArray(merged.pierceReductions)
    ...
    : parseFlexibleList(merged.pierceReductions)
        .map((entry) => {
  ...
  merged.pierceReductions = rawPierceReductions
    .map((entry) => {
      ...
      return {
        weaponType,
        pierceIgnore: Number.isFinite(pierceIgnoreRaw)
          ? Math.max(0, pierceIgnoreRaw)
          : 0,
      };
    })
  ...
  // Backwards compat: old items stored combined arms/legs; migrate to per-limb fields.
  {
    ...
    merged.protection = {
      ...
      lArm: toNonNegativeWhole(
        Object.hasOwn(srcProtection, "lArm") ? srcProtection.lArm : oldArms,
        0,
      ),
      rArm: toNonNegativeWhole(
        Object.hasOwn(srcProtection, "rArm") ? srcProtection.rArm : oldArms,
        0,
      ),
      lLeg: toNonNegativeWhole(
        Object.hasOwn(srcProtection, "lLeg") ? srcProtection.lLeg : oldLegs,
        0,
      ),
      rLeg: toNonNegativeWhole(
        Object.hasOwn(srcProtection, "rLeg") ? srcProtection.rLeg : oldLegs,
        0,
      ),
    };
  }
  ...
  merged.shields.delay = toNonNegativeWhole(merged.shields?.delay, 0);
  merged.shields.rechargeRate = toNonNegativeWhole(
    merged.shields?.rechargeRate,
    0,
  );

  merged.sourceReference.table = String(
    merged.sourceReference?.table ?? "",
  ).trim();
  merged.sourceReference.rowNumber = toNonNegativeWhole(
    merged.sourceReference?.rowNumber,
    0,
  );

  // ── Ammo-modification field normalization ─────────────────────────────────
  merged.modifierCode = String(merged.modifierCode ?? "")
    .trim()
    .toUpperCase();
  merged.modifierLabel = String(merged.modifierLabel ?? "").trim();
  const rawDamageDelta = Number(merged.damageDelta ?? 0);
  merged.damageDelta = Number.isFinite(rawDamageDelta)
    ? Math.trunc(rawDamageDelta)
    : 0;
  const rawPierceDelta = Number(merged.pierceDelta ?? 0);
  merged.pierceDelta = Number.isFinite(rawPierceDelta)
    ? Math.trunc(rawPierceDelta)
    : 0;
  ...
  const rawCompatCodes = Array.isArray(merged.compatibilityCodes)
    ...
    : String(merged.compatibilityCodes ?? "")
        .split(/[,\s]+/u)
        .filter(Boolean);
  ...
  const rawBlockedCompatCodes = Array.isArray(merged.compatibilityBlockedCodes)
    ...
    : String(merged.compatibilityBlockedCodes ?? "")
        .split(/[,\s]+/u)
        .filter(Boolean);
  ...
  merged.compatibilityMode = [
    ...
    "matrix",
    "all-shot-shells",
  ].includes(merged.compatibilityMode)
    ? merged.compatibilityMode
    : "allowlist";
  ...
  merged.addSpecialRules = normalizeStringList(
    Array.isArray(merged.addSpecialRules)
      ? merged.addSpecialRules
      : parseFlexibleList(merged.addSpecialRules),
  );
  merged.removeSpecialRules = normalizeStringList(
    Array.isArray(merged.removeSpecialRules)
      ? merged.removeSpecialRules
      : parseFlexibleList(merged.removeSpecialRules),
  );
  merged.specialRuleValues =
    merged.specialRuleValues &&
    typeof merged.specialRuleValues === "object" &&
    !Array.isArray(merged.specialRuleValues)
      ? Object.fromEntries(
  ...
  merged.costMultiplier =
    Number.isFinite(rawCostMultiplier) && rawCostMultiplier > 0
  ...
  merged.magazine = normalizeMythicMagazineData(
    merged.magazine,
    merged.storage,
    merged,
    itemName,
  ...
  const rawVehicleMount =
    merged.vehicleMount && typeof merged.vehicleMount === "object"
      ? merged.vehicleMount
      : {};
  const requestedControllerRole = String(rawVehicleMount.controllerRole ?? "")
    .trim()
    .toLowerCase();
  const controllerRole = ["operator", "gunner", "passenger"].includes(
    ...
    ? requestedControllerRole
    : "";
  const controllerPosition = toNonNegativeWhole(
    rawVehicleMount.controllerPosition,
    0,
  );
  const isMounted = Boolean(rawVehicleMount.isMounted);
  const emplacementId = String(
    rawVehicleMount.emplacementId ?? rawVehicleMount.groupId ?? "",
  ).trim();
  ...
  merged.vehicleMount = {
    ...
    linked,
    linkedCount: linked
      ? Math.max(1, toNonNegativeWhole(rawVehicleMount.linkedCount, 1))
      : 1,
  };
  ...
  try {
    if (systemData && typeof systemData === "object")
      _normCache.gear.set(systemData, merged);
  } catch (_err) {
    // ignore
  }
