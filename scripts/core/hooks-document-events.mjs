...
const MYTHIC_GRENADE_MARKER_ANIMATION_HANDLER_KEY = "_mythicGrenadeMarkerAnimationHandler";
...
function isGrenadeMarkerTileDocument(tileDocument) {
  return Boolean(tileDocument?.getFlag?.("Halo-Mythic-Foundry-Updated", "grenadeMarker"));
}

function getTileTextureSource(tileDocument) {
  return String(foundry.utils.getProperty(tileDocument ?? {}, "texture.src") ?? tileDocument?.img ?? "").trim();
}

function isLegacyOrdnanceMarkerTileDocument(tileDocument) {
  const textureSrc = getTileTextureSource(tileDocument).replace(/\\/gu, "/").toLowerCase();
  if (!textureSrc) return false;
  if (textureSrc === MYTHIC_GRENADE_MARKER_TEXTURE_SRC.toLowerCase()) return true;
  return textureSrc.includes("convergence-target.png");
}

function isClearableOrdnanceMarkerTileDocument(tileDocument) {
  return isGrenadeMarkerTileDocument(tileDocument) || isLegacyOrdnanceMarkerTileDocument(tileDocument);
}

function getSceneTileDocuments(scene) {
  if (Array.isArray(scene?.tiles?.contents)) return scene.tiles.contents;
  if (typeof scene?.tiles?.values === "function") return Array.from(scene.tiles.values());
  return [];
}

function getClearableOrdnanceMarkerIds(scene) {
  return [...new Set(getSceneTileDocuments(scene)
    .filter((tileDocument) => isClearableOrdnanceMarkerTileDocument(tileDocument))
    .map((tileDocument) => String(tileDocument?.id ?? tileDocument?._id ?? "").trim())
    .filter(Boolean))];
}
...
function isUsableGrenadeMarkerMesh(mesh) {
  return Boolean(mesh && !mesh.destroyed && mesh.transform && mesh.scale && typeof mesh.scale.set === "function");
}
...
function stopGrenadeMarkerAnimation(tileObject) {
  ...
  const animationHandler = tileObject[MYTHIC_GRENADE_MARKER_ANIMATION_HANDLER_KEY];
  if (ticker && animationHandler) {
    ticker.remove(animationHandler);
  }

  const mesh = tileObject.mesh ?? null;
  const animationState = tileObject[MYTHIC_GRENADE_MARKER_ANIMATION_STATE_KEY] ?? null;
  if (animationState && isUsableGrenadeMarkerMesh(mesh) && !tileObject.destroyed) {
    try {
...
function animateGrenadeMarkerTile(tileObject) {
  if (!isUsableGrenadeMarkerMesh(tileObject?.mesh) || !isGrenadeMarkerTileDocument(tileObject.document)) return;
  const ticker = canvas?.app?.ticker ?? null;
  if (!ticker) return;
  ...
  const baseAngle = Number(tileObject.document?.rotation ?? mesh.angle ?? 0) || 0;
  const baseAlpha = Math.max(0.2, Math.min(1, Number(tileObject.document?.alpha ?? mesh.alpha ?? 0.9) || 0.9));
  ...
  const animationHandler = () => {
    ...
    if (!isUsableGrenadeMarkerMesh(liveMesh) || tileObject.destroyed || !isGrenadeMarkerTileDocument(tileObject.document)) {
      stopGrenadeMarkerAnimation(tileObject);
      return;
    }

    const elapsedSeconds = ((globalThis.performance?.now?.() ?? Date.now()) - startTime) / 1000;
    const pulseWave = (Math.sin(elapsedSeconds * ((Math.PI * 2) / 1.5)) + 1) / 2;
    const pulseScale = 0.92 + (pulseWave * 0.35);
    const pulseAlpha = Math.max(0.35, Math.min(1, baseAlpha * (1 + (pulseWave * 0.45))));
...
function queueGrenadeMarkerAnimation(tileId) {
  ...
  schedule(() => {
    schedule(() => {
      const tileObject = canvas?.tiles?.placeables?.find((entry) => String(entry.id ?? "") === normalizedId) ?? null;
      if (tileObject) {
...
function getActiveGrenadeEvents(combat) {
  ...
  const events = combat.getFlag("Halo-Mythic-Foundry-Updated", "activeGrenadeEvents");
  return Array.isArray(events) ? events : [];
}

async function setActiveGrenadeEvents(combat, events) {
  if (!combat?.setFlag) return;
  await combat.setFlag("Halo-Mythic-Foundry-Updated", "activeGrenadeEvents", Array.isArray(events) ? events : []);
}
...
function findActiveGrenadeEventBySourceMessage(combat, sourceMessageId = "") {
  ...
  return getActiveGrenadeEvents(combat).find((entry) => {
    if (!entry || entry.resolved) return false;
    return String(entry?.sourceMessageId ?? "").trim() === normalizedSourceMessageId;
  }) ?? null;
}
...
function snapPointToGrid(scene, x = 0, y = 0) {
  ...
  const isValidPoint = (point) => point && Number.isFinite(Number(point.x)) && Number.isFinite(Number(point.y));
  ...
  if (liveGrid?.getCenterPoint && liveGrid?.getVertices) {
    try {
      ...
      const vertexPoints = Array.isArray(liveGrid.getVertices(sourcePoint)) ? liveGrid.getVertices(sourcePoint) : [];
      const candidates = [centerPoint, ...vertexPoints].filter(isValidPoint);
      if (candidates.length) {
        const snapped = candidates.reduce((best, candidate) => {
          if (!best) return candidate;
          return distanceSquared(candidate, sourcePoint) < distanceSquared(best, sourcePoint) ? candidate : best;
        }, null);
        if (snapped) {
          return {
  ...
  if (liveGrid?.isSquare) {
    ...
    const centerPoint = { x: snappedX + (gridSize / 2), y: snappedY + (gridSize / 2) };
    const cornerPoints = [
...
async function promptGrenadeLocation(event, title = "Grenade Target Location", options = {}) {
  ...
  const maxDistanceMeters = Number.isFinite(maxDistanceMetersRaw) ? Math.max(0, Math.floor(maxDistanceMetersRaw)) : NaN;
  const originPoint = options?.originPoint && Number.isFinite(Number(options.originPoint.x)) && Number.isFinite(Number(options.originPoint.y))
    ? { x: Number(options.originPoint.x), y: Number(options.originPoint.y) }
    : null;
  const validateLocationChoice = (choice) => {
    if (!scene || !originPoint || !Number.isFinite(maxDistanceMeters)) return true;
    const distanceMeters = measureSceneDistanceMeters(scene, originPoint, choice);
    if (!Number.isFinite(distanceMeters) || distanceMeters <= (maxDistanceMeters + 1e-6)) {
      return true;
    }
    ui.notifications?.warn(`You can only throw the grenade ${maxDistanceMeters} meters`);
    return false;
  };
  ...
  if (scene && stage && view) {
    return new Promise((resolve) => {
      ...
      const onPointerDown = (pointerEvent) => {
        ...
        const local = resolveCanvasPointFromClient(pointerEvent?.clientX, pointerEvent?.clientY);
        if (!local) {
          keepCrosshair();
      ...
      keepCrosshair();
      const alignmentHint = !scene?.grid || Number(scene.grid.type ?? 0) <= 0
      ...
      ui.notifications?.info(`${title}: click on the map to place it. ${alignmentHint} Press Escape or right-click to cancel.`);
      ...
      buttons: [
        {
          ...
          label: "Set Location",
          callback: () => {
            const x = Number(document.getElementById("mythic-grenade-x")?.value ?? currentX);
            const y = Number(document.getElementById("mythic-grenade-y")?.value ?? currentY);
            return {
...
async function clearGrenadeExplosionVisualsByData({ sceneId = null, markerId = null, templateIds = [] } = {}) {
  ...
  const normalizedTemplateIds = normalizeGrenadeExplosionTemplateIds(templateIds);
  ...
  if (normalizedTemplateIds.length) {
    try {
      await scene.deleteEmbeddedDocuments("MeasuredTemplate", normalizedTemplateIds);
...
function parseBlastKillRadii(attackData = {}) {
  const specialRuleValues = (attackData?.weaponSpecialRuleValues && typeof attackData.weaponSpecialRuleValues === "object" && !Array.isArray(attackData.weaponSpecialRuleValues))
...
function isCookGrenadeEvent(event = {}) {
  ...
  const cookState = String(event?.cookState ?? "").trim().toLowerCase();
  return action === "cook"
...
function getTokenCenterFromDocument(tokenDocument) {
  ...
  if (Number.isFinite(Number(objectCenter?.x)) && Number.isFinite(Number(objectCenter?.y))) {
    return {
  ...
  const heightPixels = Math.max(1, Number(tokenDocument?.height ?? 1)) * gridSize;
  if (![x, y, widthPixels, heightPixels].every((value) => Number.isFinite(value))) return null;
  return {
    x: x + (widthPixels / 2),
    y: y + (heightPixels / 2)
  };
...
async function syncCookEventPositionsForTokenDocument(tokenDocument, { clearTokenId = false } = {}) {
  ...
  const nextEvents = activeEvents.map((entry) => {
    ...
    if (ownerActorId !== tokenActorId && (!ownerTokenId || ownerTokenId !== tokenId)) return entry;
    ...
    nextEntry.sceneId = String(tokenDocument.parent?.id ?? nextEntry.sceneId ?? canvas?.scene?.id ?? "").trim() || nextEntry.sceneId;
    nextEntry.tokenName = String(tokenDocument.name ?? nextEntry.tokenName ?? "").trim() || nextEntry.tokenName;
...
function getGrenadeOwnerToken(event = {}) {
  ...
  if (tokenId) {
    const liveToken = canvas?.tokens?.get?.(tokenId)
      ?? canvas?.tokens?.placeables?.find((token) => String(token?.id ?? "") === tokenId)
      ?? null;
  ...
  return canvas?.tokens?.placeables?.find((token) => String(token?.actor?.id ?? "") === String(event?.ownerActorId ?? "")) ?? null;
}
...
function isGrenadeCookOwnerUnavailable(event, actor = null, token = null) {
  ...
  const combatant = token?.combatant
    ?? game.combat?.combatants?.find((entry) => {
      ...
      return actorId === String(actor?.id ?? "") && (!token?.id || tokenId === String(token.id ?? ""));
    })
    ?? null;
...
async function removeActiveGrenadeEvent(combat, eventId) {
  ...
  const activeEvents = getActiveGrenadeEvents(combat);
  await setActiveGrenadeEvents(combat, activeEvents.filter((entry) => String(entry?.id ?? "") !== normalizedEventId));
}
...
function buildGrenadeCookThrowResultContent({
  ...
  const esc = foundry.utils.escapeHTML;
  const defaultThrowText = defaultThrowMeters > 0 ? ` / default ${Math.floor(defaultThrowMeters)}m` : "";
...
function buildGrenadeCookScatterResultContent({
  ...
  const esc = foundry.utils.escapeHTML;
  const capHtml = maxDistanceMeters > 0
...
function buildGrenadeCookDamagePromptContent({ event = null, reason = "" } = {}) {
  const esc = foundry.utils.escapeHTML;
...
function buildGrenadeCookAutoExplosionContent({ event = null, reason = "", x = 0, y = 0 } = {}) {
  const esc = foundry.utils.escapeHTML;
...
async function updateGrenadeCookPromptMessage(promptMessage, { content = null, resolved = null } = {}) {
  if (!promptMessage) return;
  if (typeof content === "string" && typeof promptMessage.update === "function") {
    await promptMessage.update({ content });
  }
  if (resolved === null || !promptMessage?.setFlag) return;
  const currentPrompt = promptMessage.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookPrompt");
  const nextPrompt = currentPrompt && typeof currentPrompt === "object"
    ? foundry.utils.deepClone(currentPrompt)
    : {};
  nextPrompt.resolved = Boolean(resolved);
  await promptMessage.setFlag("Halo-Mythic-Foundry-Updated", "grenadeCookPrompt", nextPrompt);
}
...
async function updateGrenadeCookStageMessage(message, flagKey, { content = null, updates = null } = {}) {
...
async function upsertGrenadeCookStageMessage(combat, event, {
  messageIdField = "",
  flagKey = "",
  content = "",
  flagData = {}
  ...
  const ownerActor = game.actors.get(String(event?.ownerActorId ?? "").trim()) ?? null;
  const currentMessageId = String(event?.[messageIdField] ?? "").trim();
  let stageMessage = currentMessageId ? (game.messages?.get?.(currentMessageId) ?? null) : null;
  if (!stageMessage) {
  ...
  if (!stageMessage) {
    ...
    const controlFlag = outsideCombat
      ? {
        ...
        noticeData: {
          attackerName: String(nextFlagData.weaponName ?? nextFlagData.ownerActorId ?? "").trim(),
    ...
    stageMessage = await ChatMessage.create({
      ...
      }, controlFlag ? { "Halo-Mythic-Foundry-Updated": Object.assign({ [flagKey]: nextFlagData }, { grenadeEventControl: controlFlag }) } : {})
    });
  } else {
    ...
    if (!combat && stageMessage?.setFlag) {
      ...
      await stageMessage.setFlag("Halo-Mythic-Foundry-Updated", "grenadeEventControl", controlFlag);
    }
  }

  if (combat && stageMessage?.id) {
    const updatedEvent = await updateActiveGrenadeEvent(combat, event.id, (entry) => {
...
async function ensureGrenadeCookPromptMessage(combat, event) {
  ...
  let promptMessage = currentPromptId ? (game.messages?.get?.(currentPromptId) ?? null) : null;
  if (!promptMessage) {
    promptMessage = findLatestGrenadeMessageByFlag("grenadeCookPrompt", event?.id);
  }
  ...
  if (!promptMessage) {
    ...
    const controlFlag = outsideCombat
      ? {
        ...
        eventData: foundry.utils.deepClone(event),
        noticeData: {
          attackerName: String(promptFlag.weaponName ?? promptFlag.ownerActorId ?? "").trim(),
    ...
    promptMessage = await ChatMessage.create({
      ...
      type: CONST.CHAT_MESSAGE_STYLES.OTHER,
      flags: controlFlag
        ? { "Halo-Mythic-Foundry-Updated": Object.assign({ grenadeCookPrompt: promptFlag }, { grenadeEventControl: controlFlag }) }
    ...
    if (!combat && promptMessage?.setFlag) {
      const controlFlag = {
        eventId: String(promptMessage.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookPrompt")?.eventId ?? "").trim(),
        combatId: String(promptMessage.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookPrompt")?.combatId ?? "").trim(),
        outsideCombat: true,
        eventData: foundry.utils.deepClone(event),
        noticeData: {
          attackerName: String(promptMessage.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookPrompt")?.weaponName ?? "").trim(),
          weaponName: String(promptMessage.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookPrompt")?.weaponName ?? "").trim()
        }
      };
      await promptMessage.setFlag("Halo-Mythic-Foundry-Updated", "grenadeEventControl", controlFlag);
    }
  }

  const updatedEvent = await updateActiveGrenadeEvent(combat, event.id, (entry) => {
    entry.cookPromptMessageId = String(promptMessage?.id ?? "").trim() || null;
...
function computeDeferredGrenadeThrowProfile(actor, weaponData = {}) {
  ...
  const mythicStrength = Math.max(0, toNonNegativeWhole(actor?.system?.mythic?.characteristics?.str, 0));
  ...
  const throwWeightKgRaw = Number(weaponData?.weightKg ?? weaponData?.weightPerRoundKg ?? 0);
  ...
  const wieldingTypeTag = String(weaponData?.wieldingType ?? "").trim().toUpperCase();
  const rulesText = String(weaponData?.specialRules ?? "").toUpperCase();
  const weaponTagKeys = Array.isArray(weaponData?.weaponTagKeys)
    ? weaponData.weaponTagKeys.map((entry) => String(entry ?? "").trim().toUpperCase())
    : [];
  const hasOneHandThrowTag = String(weaponData?.ammoMode ?? "").trim().toLowerCase() === "grenade"
    || String(weaponData?.equipmentType ?? "").trim().toLowerCase() === "explosives-and-grenades"
...
function computeDeferredGrenadeThrowProfileFromAttackData(attackData = {}) {
  ...
  const multiplierAfterWeight = Number(attackData?.throwMultiplierAfterWeight ?? NaN);
  const usesOneHandPenalty = attackData?.throwUsesOneHandPenalty;
  if (!Number.isFinite(strengthPower) || !Number.isFinite(multiplierAfterWeight) || typeof usesOneHandPenalty !== "boolean") {
    return null;
  }
  ...
  const resolveDistanceFromMultiplier = (multiplierValue = 0) => {
    ...
    const baseMeters = multiplierValue < 0
      ? (negativeBaseMeters >= 0.5 ? Math.max(1, Math.floor(negativeBaseMeters)) : 0)
      : Math.max(0, Math.floor(strengthPower * multiplierValue));
    const handedMeters = usesOneHandPenalty ? Math.floor(baseMeters / 2) : baseMeters;
    return Math.max(0, handedMeters);
  };
...
async function setGrenadeMessageData(message, event, action = "throw", { outsideCombat = false } = {}) {
  if (!message?.setFlag || !event) return;

  const normalizedAction = String(action ?? event?.action ?? "throw").trim().toLowerCase();
  await message.setFlag("Halo-Mythic-Foundry-Updated", "grenadeData", {
    id: String(event?.id ?? "").trim() || null,
    armed: true,
    action: normalizedAction,
    cookState: normalizedAction === "cook" ? String(event?.cookState ?? "armed").trim().toLowerCase() || "armed" : "",
    outsideCombat: Boolean(outsideCombat),
    locationLabel: normalizedAction === "cook"
      ? "Cooking started"
...
async function rollDeferredGrenadeDamage(event) {
  const weaponData = (event?.attackData?.deferredGrenadeWeaponData && typeof event.attackData.deferredGrenadeWeaponData === "object")
    ? event.attackData.deferredGrenadeWeaponData
    : {};
  const weaponState = (event?.attackData?.deferredGrenadeWeaponState && typeof event.attackData.deferredGrenadeWeaponState === "object")
  ...
  const flatDamage = Number(weaponData?.damage?.baseDamage ?? 0) + Number(weaponState?.damageModifier ?? 0);
  const formulaParts = [];
  if (d10Count > 0) formulaParts.push(`${d10Count}d10`);
  if (d5Count > 0) formulaParts.push(`${d5Count}d5`);
  if (flatDamage !== 0 || !formulaParts.length) formulaParts.push(String(flatDamage));
  ...
  const blastPierce = Math.max(0, Number(weaponData?.damage?.pierce ?? event?.attackData?.damagePierce ?? 0));
  return {
    damageRoll,
    blastDamage,
...
async function ensureGrenadeCookDamagePromptMessage(combat, event, { reason = "" } = {}) {
  ...
  if (combat) {
    const updatedEvent = await updateActiveGrenadeEvent(combat, nextEvent.id, (entry) => {
...
async function autoResolveCookExplosion(combat, event, { promptMessage = null, reason = "" } = {}) {
  let nextEvent = foundry.utils.deepClone(event ?? {});
  ...
  if (combat) {
    const syncedEvent = await updateActiveGrenadeEvent(combat, nextEvent.id, (entry) => {
  ...
  const resolvedPromptMessage = promptMessage
    ?? (String(nextEvent?.cookPromptMessageId ?? "").trim()
      ? (game.messages?.get?.(String(nextEvent.cookPromptMessageId ?? "").trim()) ?? null)
      : null);
...
async function createGrenadeEvent(message, attackData = {}, action = "throw", options = {}) {
  const combat = game.combat;
  const normalizedAction = String(action ?? "throw").trim().toLowerCase();
  const sourceMessageId = String(message?.id ?? "").trim();
  const scatterRequired = Number(attackData?.dosValue ?? 0) < 0;
  const placementMode = String(options?.placementMode ?? "").trim().toLowerCase();
  ...
  const resolvedThrowRangeMeters = Math.max(0, Math.floor(Number(attackData?.throwRangeMeters ?? 0)));
  const defaultThrowRangeMeters = Math.max(0, Math.floor(Number(attackData?.throwRangeMaxMeters ?? 0)));
  const allowScatter = options?.allowScatter ?? scatterRequired;
  const allowMove = options?.allowMove ?? (normalizedAction === "throw");

  if (sourceMessageId && MYTHIC_PENDING_GRENADE_EVENT_MESSAGE_IDS.has(sourceMessageId)) {
    return hasCombat ? findActiveGrenadeEventBySourceMessage(combat, sourceMessageId) : null;
  }

  if (hasCombat && sourceMessageId) {
    const existingEvent = findActiveGrenadeEventBySourceMessage(combat, sourceMessageId);
    if (existingEvent) {
      await setGrenadeMessageData(message, existingEvent, existingEvent.action, {
        outsideCombat: false
      });
  ...
  try {
    ...
    const grenadeEvent = {
      ...
      ammoMode: String(attackData.ammoMode ?? "").trim().toLowerCase(),
      ...
      detonateAt: hasCombat ? { round: currentRound + timerDelayRounds, turn: currentTurn } : null,
      timerDelayRounds,
      action: normalizedAction,
      armed: true,
      resolved: false,
    ...
    if (normalizedAction === "throw" && !skipThrowPrompt) {
      ...
      try {
        const locationChoice = await promptGrenadeLocation(grenadeEvent, "Throw Grenade - Target Location", {
          originPoint: ownerToken?.center ?? origin,
          maxDistanceMeters: resolvedThrowRangeMeters
        });
        if (!locationChoice) return null;
        const snapped = snapPointToGrid(canvas?.scene, locationChoice.x, locationChoice.y);
    ...
    } else if (normalizedAction === "throw" && autoPlaceAtThrower && game.user.isGM) {
      grenadeEvent.markerId = await createOrUpdateGrenadeMarker(grenadeEvent);
    }

    if (hasCombat) {
      const existingEvent = sourceMessageId ? findActiveGrenadeEventBySourceMessage(combat, sourceMessageId) : null;
      if (existingEvent) {
        await setGrenadeMessageData(message, existingEvent, existingEvent.action, {
          outsideCombat: false
        });
    ...
    await setGrenadeMessageData(message, resolvedGrenadeEvent, normalizedAction, {
      outsideCombat: !hasCombat
    });
    await message.render(true);

    const title = normalizedAction === "cook"
      ? "Grenade Cooking"
      : (hasCombat ? "Grenade Armed" : "Grenade Placed");
    const bodyText = normalizedAction === "cook"
    ...
    const placementControl = normalizedAction === "throw" && (Boolean(allowMove) || scatterPending)
      ? {
        ...
        outsideCombat: !hasCombat,
        eventData: hasCombat ? null : foundry.utils.deepClone(resolvedGrenadeEvent),
        noticeData: {
          attackerName: String(attackData.attackerName ?? "").trim() || "Actor",
          weaponName: String(attackData.weaponName ?? "").trim() || "Grenade"
        }
      }
      : null;
    const noticeFlags = {
    ...
    if (normalizedAction !== "cook") {
      await ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ actor: ownerActor }),
        content: normalizedAction === "throw"
...
async function createGrenadeScatterEventFromThrowFailure(message, attackData) {
  ...
  const alreadyResolved = Boolean(message.getFlag("Halo-Mythic-Foundry-Updated", "failedThrowScatterResolved"));
  if (alreadyResolved) return null;
  await message.setFlag("Halo-Mythic-Foundry-Updated", "failedThrowScatterResolved", true);

  const ownerToken = canvas?.tokens?.placeables?.find((token) => token?.actor?.id === attackData.attackerId) ?? null;
  const resolvedThrowRange = Math.max(0, Math.floor(Number(attackData?.throwRangeMeters ?? 0)));
  const defaultThrowRange = Math.max(0, Math.floor(Number(attackData?.throwRangeMaxMeters ?? 0)));
  const targetTokenId = String(attackData?.targetTokenId ?? "").trim();
  const targetToken = targetTokenId
    ? (canvas?.tokens?.get?.(targetTokenId) ?? canvas?.tokens?.placeables?.find((token) => String(token?.id ?? "") === targetTokenId) ?? null)
    : null;

  const hasTargetPoint = Boolean(targetToken?.center);
  const initialPoint = (resolvedThrowRange > 0 && hasTargetPoint)
    ? {
  ...
  if (!game.combat) {
    const dof = Math.max(0, Math.floor(Math.abs(Math.min(0, Number(attackData?.dosValue ?? 0)))));
    const scatterDiceCount = Math.max(1, 1 + dof);
    const scatterMultiplierRaw = Number(attackData?.grenadeScatterMultiplier ?? 1);
    const scatterMultiplier = Number.isFinite(scatterMultiplierRaw) ? Math.max(1, Math.floor(scatterMultiplierRaw)) : 1;
    const directionRoll = await new Roll("1d10").evaluate();
    const scatterDistanceRoll = await new Roll(`${scatterDiceCount}d10`).evaluate();
    const baseDistanceMeters = Math.max(0, Math.floor(Number(scatterDistanceRoll.total ?? 0) / 2));
    const uncappedDistanceMeters = Math.max(0, Math.floor(baseDistanceMeters * scatterMultiplier));
    const maxDistanceMeters = Math.max(defaultThrowRange, resolvedThrowRange);
    const distanceMeters = maxDistanceMeters > 0
    ...
    const scatterX = Number(initialPoint.x ?? 0) + (direction.dx * distanceMeters);
    const scatterY = Number(initialPoint.y ?? 0) + (direction.dy * distanceMeters);
    const snapped = snapPointToGrid(canvas?.scene, scatterX, scatterY);
    const originLabel = resolvedThrowRange <= 0 || !hasTargetPoint
      ? "Thrower"
    ...
    return { direction: direction.label, distanceMeters, x: snapped.x, y: snapped.y };
  }
  ...
  const snapped = snapPointToGrid(canvas?.scene, initialPoint.x, initialPoint.y);
  const updatedEvent = await updateActiveGrenadeEvent(game.combat, grenadeEvent.id, (entry) => {
    entry.x = snapped.x;
    entry.y = snapped.y;
  });
...
async function rollGrenadeScatterResult(originPoint, attackData = {}, options = {}) {
  const scene = game.scenes?.get(String(options?.sceneId ?? "")) ?? canvas?.scene;
  const dof = Math.max(0, Math.floor(Math.abs(Math.min(0, Number(attackData?.dosValue ?? 0)))));
  const scatterDiceCount = Math.max(1, 1 + dof);
  const scatterMultiplierRaw = Number(attackData?.grenadeScatterMultiplier ?? 1);
  const scatterMultiplier = Number.isFinite(scatterMultiplierRaw) ? Math.max(1, Math.floor(scatterMultiplierRaw)) : 1;
  const throwFromToken = options?.throwFromToken ?? Boolean(attackData?.throwDropsAtThrower);
  const directionRoll = await new Roll("1d10").evaluate();
  const scatterDistanceRoll = await new Roll(`${scatterDiceCount}d10`).evaluate();
  const baseDistanceMeters = Math.max(0, Math.floor(Number(scatterDistanceRoll.total ?? 0) / 2));
  const uncappedDistanceMeters = Math.max(0, Math.floor(baseDistanceMeters * scatterMultiplier));
  const maxDistanceMetersRaw = Number(options?.maxDistanceMeters ?? attackData?.throwScatterCapMeters ?? attackData?.throwRangeMeters ?? attackData?.throwRangeMaxMeters ?? 0);
  const maxDistanceMeters = Number.isFinite(maxDistanceMetersRaw) && maxDistanceMetersRaw > 0
...
async function applyGrenadeScatterToEvent(grenadeEvent, options = {}) {
  ...
  const combat = options?.combat
    ?? ((game.combat && String(game.combat.id ?? "") === String(grenadeEvent.combatId ?? "")) ? game.combat : null);
  ...
  if (combat) {
    const updatedEvent = await updateActiveGrenadeEvent(combat, grenadeEvent.id, (entry) => {
  ...
  if (game.user.isGM) {
    ...
    if (markerId) {
      if (combat && String(nextEvent.markerId ?? "") !== String(markerId)) {
        const updatedWithMarker = await updateActiveGrenadeEvent(combat, grenadeEvent.id, (entry) => {
          entry.markerId = markerId;
        });
...
function getGrenadeControlFlag(message) {
  const control = message?.getFlag?.("Halo-Mythic-Foundry-Updated", "grenadeEventControl");
...
async function updateGrenadeControlFlag(message, updater) {
  ...
  await message.setFlag("Halo-Mythic-Foundry-Updated", "grenadeEventControl", nextControl);
  return nextControl;
}

async function getGrenadeEventForControl(combat, eventId, controlMessage = null) {
  const normalizedEventId = String(eventId ?? "").trim();
  ...
  const eventData = control?.eventData && typeof control.eventData === "object"
    ? foundry.utils.deepClone(control.eventData)
    : null;
  if (eventData && String(eventData.id ?? "").trim() === normalizedEventId) return eventData;

  // Fallback: attempt to reconstruct minimal event data from the message flags
  if (controlMessage) {
    try {
      const msgFlags = (controlMessage?.data?.flags) || {};
      const gmFlags = (msgFlags["Halo-Mythic-Foundry-Updated"] && typeof msgFlags["Halo-Mythic-Foundry-Updated"] === "object") ? msgFlags["Halo-Mythic-Foundry-Updated"] : {};
      const grenadeData = controlMessage.getFlag("Halo-Mythic-Foundry-Updated", "grenadeData") ?? null;
      const cookPrompt = controlMessage.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookPrompt") ?? null;
      const cookDamagePrompt = controlMessage.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookDamagePrompt") ?? null;
      const attackData = controlMessage.getFlag("Halo-Mythic-Foundry-Updated", "attackData") ?? null;
      const candidateId = String(grenadeData?.id ?? cookPrompt?.eventId ?? cookDamagePrompt?.eventId ?? "").trim();
      if (candidateId && candidateId === normalizedEventId) {
        const built = {
          id: normalizedEventId,
          ownerActorId: String(attackData?.attackerId ?? cookPrompt?.ownerActorId ?? "").trim() || null,
          ownerName: String(attackData?.attackerName ?? cookPrompt?.ownerActorId ?? "").trim() || null,
          weaponId: String(attackData?.weaponId ?? "").trim() || null,
          weaponName: String(attackData?.weaponName ?? cookPrompt?.weaponName ?? "").trim() || null,
          ...
          sceneId: String(attackData?.sceneId ?? grenadeData?.sceneId ?? canvas?.scene?.id ?? "").trim() || null
        };
        ...
        try {
          ...
          const scene = game.scenes?.get(String(built.sceneId ?? "")) ?? canvas?.scene ?? null;
          let tile = null;

          // Try explicit marker id lookup first (if present)
          if (markerId && scene) {
            try {
              tile = scene.getEmbeddedDocument ? scene.getEmbeddedDocument("Tile", markerId) : (scene.tiles?.get ? scene.tiles.get(markerId) : null);
            } catch (err) {
              tile = (scene.tiles && typeof scene.tiles.get === "function") ? scene.tiles.get(markerId) : null;
            }
          }
          ...
          if (!tile && scene) {
            try {
              ...
              for (const td of tileDocs) {
                try {
                  const tFlag = td?.getFlag ? td.getFlag("Halo-Mythic-Foundry-Updated", "grenadeEventId") : (td?.flags?.["Halo-Mythic-Foundry-Updated"]?.grenadeEventId);
          ...
          if (!tile && canvas?.scene && scene && String(canvas.scene.id ?? "") === String(scene.id ?? "")) {
            try {
              tile = canvas.scene.tiles.placeables?.find((t) => {
                try {
                  return String(t.document?.getFlag?.("Halo-Mythic-Foundry-Updated", "grenadeEventId") ?? "").trim() === normalizedEventId
                    || String(t.document?.flags?.["Halo-Mythic-Foundry-Updated"]?.grenadeEventId ?? "").trim() === normalizedEventId;
                } catch (e) {
                  return false;
                }
              }) ?? null;
          ...
          if (tile) {
            ...
            built.markerId = String(tile.id ?? tile._id ?? built.markerId ?? "").trim() || built.markerId;
            ...
            } else if (tile?.center && Number.isFinite(Number(tile.center.x)) && Number.isFinite(Number(tile.center.y))) {
            ...
            const coordInvalid = !Number.isFinite(Number(built.x)) && !Number.isFinite(Number(built.y)) || (Number(built.x ?? 0) === 0 && Number(built.y ?? 0) === 0);
            if (coordInvalid && scene) {
              try {
                // Prefer target tokens if provided
                const targetIds = Array.isArray(built.attackData?.targetTokenIds) && built.attackData.targetTokenIds.length ? built.attackData.targetTokenIds : [];
                let tokenObject = null;
                if (targetIds.length) {
                  tokenObject = (scene.tokens?.get ? scene.tokens.get(targetIds[0]) : null) ?? canvas?.tokens?.get?.(targetIds[0]) ?? null;
                }
                // Fallback to single targetTokenId or attacker token
                if (!tokenObject && String(built.attackData?.targetTokenId ?? "").trim()) {
                  tokenObject = (scene.tokens?.get ? scene.tokens.get(String(built.attackData.targetTokenId)) : null) ?? canvas?.tokens?.get?.(String(built.attackData.targetTokenId)) ?? null;
                }
                if (!tokenObject && String(built.ownerActorId ?? "").trim()) {
                  tokenObject = (scene.tokens?.find ? Array.from(scene.tokens.values()).find((t) => String(t.actor?.id ?? "") === String(built.ownerActorId ?? "")) : null)
                    ?? canvas?.tokens?.placeables?.find((t) => String(t.actor?.id ?? "") === String(built.ownerActorId ?? "")) ?? null;
                }
                if (tokenObject) {
                  const center = tokenObject.center ?? (tokenObject.document ? getTokenCenterFromDocument(tokenObject.document) : null);
...
async function syncGrenadeControlMessage(controlMessage, nextEvent, controlUpdates = {}) {
  if (!controlMessage) return null;
  const nextControl = await updateGrenadeControlFlag(controlMessage, (control) => {
    Object.assign(control, controlUpdates ?? {});
    const isOutsideCombat = Boolean(control.outsideCombat || !String(control.combatId ?? "").trim());
    if (isOutsideCombat) {
      control.eventData = nextEvent ? foundry.utils.deepClone(nextEvent) : null;
    }
  });
  if (nextControl?.noticeData && nextEvent && typeof controlMessage.update === "function") {
    await controlMessage.update({
      content: buildGrenadePlacementNoticeContent({
        ...
        outsideCombat: Boolean(nextControl.outsideCombat || !String(nextControl.combatId ?? "").trim()),
        event: nextEvent,
...
async function updateActiveGrenadeEvent(combat, eventId, updater) {
  ...
  const idx = events.findIndex((entry) => String(entry?.id ?? "") === String(eventId ?? ""));
...
async function rollAndApplyGrenadeScatter(combat, eventId, options = {}) {
  ...
  const updatedEvent = await getGrenadeEventForControl(combat, eventId, controlMessage);
  ...
  if (scatteredEvent && controlMessage) {
    await syncGrenadeControlMessage(controlMessage, scatteredEvent, { allowScatter: false });
  }
...
async function moveGrenadeMarkerManually(combat, eventId, options = {}) {
  const controlMessage = options?.controlMessage ?? null;
  const currentEvent = await getGrenadeEventForControl(combat, eventId, controlMessage);
  if (!currentEvent) return;
  const minimizedApps = await minimizeOpenSheetApplications();
  let nextPoint = null;
  try {
    nextPoint = await promptGrenadeLocation(currentEvent, "Move Grenade Marker");
  } finally {
    await restoreMinimizedSheetApplications(minimizedApps);
  }
  if (!nextPoint) return;
  ...
  if (nextEvent && game.user.isGM) {
    ...
    if (markerId && String(nextEvent.markerId ?? "") !== String(markerId)) {
      if (combat) {
        const updatedWithMarker = await updateActiveGrenadeEvent(combat, eventId, (entry) => {
          entry.markerId = markerId;
        });
        if (updatedWithMarker) {
          nextEvent = updatedWithMarker;
...
async function resolveCookThrowAtTurnStart(event, { combat = game.combat, promptMessage = null } = {}) {
  let nextEvent = foundry.utils.deepClone(event ?? {});
  ...
  if (isGrenadeCookOwnerUnavailable(nextEvent, actor, actorToken)) {
    ...
    return autoResolveCookExplosion(combat, nextEvent, { promptMessage, reason });
  }

  const storedWeaponData = (nextEvent?.attackData?.deferredGrenadeWeaponData && typeof nextEvent.attackData.deferredGrenadeWeaponData === "object")
    ? nextEvent.attackData.deferredGrenadeWeaponData
    : null;
  const sourceWeaponItem = actor?.items?.get?.(String(nextEvent.weaponId ?? "")) ?? null;
  const deferredWeaponData = storedWeaponData
    ?? (sourceWeaponItem ? normalizeGearSystemData(sourceWeaponItem.system ?? {}, sourceWeaponItem.name ?? "") : {});
  const throwProfile = computeDeferredGrenadeThrowProfileFromAttackData(nextEvent?.attackData)
  ...
  try {
    ...
    ui.notifications?.error("Failed to open Attack Modifiers dialog. See console for details.");
    return null;
  }
  if (attackMods === null) {
    ui.notifications?.info("Attack modifiers dialog cancelled; throw resolution aborted.");
    return null;
  }
  ...
  const throwTarget = wfmValue
    + Number(attackMods?.toHitMod ?? 0)
    + Number(nextEvent?.attackData?.throwBaseToHitMod ?? deferredWeaponData?.baseToHitModifier ?? 0)
    + Number(nextEvent?.attackData?.throwStateToHitMod ?? nextEvent?.attackData?.deferredGrenadeWeaponState?.toHitModifier ?? 0)
  ...
  const grenadeDof = Math.max(0, Math.floor(Math.abs(Math.min(0, Number(throwDos ?? 0)))));
  const baseThrowMultiplier = Number.isFinite(Number(nextEvent?.attackData?.throwMultiplierAfterWeight))
  ...
  const defaultThrowMeters = Number.isFinite(Number(nextEvent?.attackData?.throwRangeMaxMeters))
    ? Math.max(0, Math.floor(Number(nextEvent.attackData.throwRangeMaxMeters)))
    : Math.max(0, Math.floor(Number(throwProfile.resolveDistanceFromMultiplier?.(baseThrowMultiplier) ?? 0)));
  const throwDistanceMeters = Math.max(0, Math.floor(Number(throwProfile.resolveDistanceFromMultiplier?.(throwResolvedMultiplier) ?? 0)));
  const throwRawDistanceMeters = Math.max(0, Number(throwProfile.resolveRawDistanceFromMultiplier?.(throwResolvedMultiplier) ?? 0));
  const throwDropsAtThrower = throwResolvedMultiplier <= -10 || throwRawDistanceMeters < 0.5 || throwDistanceMeters <= 0;
  const throwScatterCapMeters = throwDropsAtThrower
    ? Math.max(0, Math.floor(Number(throwProfile.resolveDistanceFromMultiplier?.(-1) ?? 0)))
    : throwDistanceMeters;

  const throwerPoint = actorToken?.center
    ? { x: Number(actorToken.center.x ?? 0), y: Number(actorToken.center.y ?? 0) }
  ...
  nextEvent.throwTest = {
    ...
    throwMath: {
      ...
      strengthPower: Number(throwProfile?.strengthPower ?? nextEvent?.attackData?.throwStrengthPower ?? 0),
      throwWeightKg: Number(throwProfile?.throwWeightKg ?? nextEvent?.attackData?.throwWeightKg ?? 0),
      weightPenalty: Number(throwProfile?.weightPenalty ?? nextEvent?.attackData?.throwWeightPenalty ?? 0),
      throwStrengthBonus: Number(nextEvent?.attackData?.throwStrengthBonus ?? throwProfile?.throwStrengthBonus ?? 0),
      baseMultiplier: Number(baseThrowMultiplier ?? 0),
      grenadeDof: Number(grenadeDof ?? 0),
      throwResolvedMultiplier: Number(throwResolvedMultiplier ?? 0),
      usesOneHandPenalty: Boolean(nextEvent?.attackData?.throwUsesOneHandPenalty ?? false)
    }
  };
  ...
  const throwResultUpsert = await upsertGrenadeCookStageMessage(combat, nextEvent, {
  ...
  if (!throwDropsAtThrower) {
    ...
    try {
      targetChoice = await promptGrenadeLocation(nextEvent, "Cooked Grenade - Throw Location", {
    ...
    const resolvedPoint = targetChoice
      ...
      : throwerPoint;
    if (!targetChoice) {
      ui.notifications?.info("Throw location cancelled. The grenade resolves from the thrower’s position.");
    }
...
async function resolveCookScatterAtTurnStart(event, { combat = game.combat, throwResultMessage = null } = {}) {
  let nextEvent = foundry.utils.deepClone(event ?? {});
  ...
  if (nextEvent.scatterResolved) {
    ui.notifications?.info("Scatter has already been resolved for this cooked grenade.");
    return nextEvent;
  }
  ...
  const scatterResult = await rollGrenadeScatterResult({
    ...
    maxDistanceMeters: Number(nextEvent?.throwTest?.throwScatterCapMeters ?? nextEvent?.attackData?.throwScatterCapMeters ?? 0),
    throwFromToken: Boolean(nextEvent?.throwTest?.throwDropsAtThrower ?? nextEvent?.attackData?.throwDropsAtThrower)
  });
  ...
  const scatterMessageUpsert = await upsertGrenadeCookStageMessage(combat, nextEvent, {
  ...
  const throwResultMessageId = String(nextEvent?.throwResultMessageId ?? "").trim();
  const resolvedThrowResultMessage = throwResultMessage
    ?? (throwResultMessageId ? (game.messages?.get?.(throwResultMessageId) ?? null) : null);
  if (resolvedThrowResultMessage) {
    await updateGrenadeCookStageMessage(resolvedThrowResultMessage, "grenadeCookThrowResult", {
      content: buildGrenadeCookThrowResultContent({
        event: nextEvent,
        ...
        throwDistanceMeters: Number(nextEvent?.throwTest?.throwDistanceMeters ?? 0),
        defaultThrowMeters: Number(nextEvent?.throwTest?.defaultThrowMeters ?? 0),
        finalX: Number(nextEvent.x ?? 0),
        finalY: Number(nextEvent.y ?? 0),
        autoPlacedAtThrower: Boolean(nextEvent?.throwTest?.throwDropsAtThrower),
        throwMath: nextEvent?.throwTest?.throwMath ?? null,
...
async function resolveCookTestAtTurnStart(event, { combat = game.combat } = {}) {
  let nextEvent = foundry.utils.deepClone(event ?? {});
  const actor = game.actors.get(String(nextEvent.ownerActorId ?? "")) ?? null;
  const actorToken = getGrenadeOwnerToken(nextEvent);
  if (actorToken?.center) {
  ...
  if (nextEvent.cookResolved) {
    ui.notifications?.info("The cook test has already been resolved for this grenade.");
    return nextEvent;
  }
  ...
  const wfmValue = toNonNegativeWhole(characteristics?.wfm, 0);
  const demolitionsTier = String(actor?.system?.skills?.base?.demolitions?.tier ?? "untrained").trim().toLowerCase();
  const demolitionsBonus = demolitionsTier === "plus20" ? 20 : demolitionsTier === "plus10" ? 10 : demolitionsTier === "trained" ? 5 : 0;
  const hasMindTimer = (actor?.items ?? []).some((entry) => entry.type === "ability" && String(entry.name ?? "").trim().toLowerCase() === "mind timer");
    ...
    buttons: [
      {
        ...
        callback: () => String(document.getElementById("mythic-cook-skill")?.value ?? "agi")
      },
      {
        action: "cancel",
        label: "Cancel",
  ...
  const chosenKey = ["agi", "int", "wfm"].includes(String(cookSkillChoice)) ? String(cookSkillChoice) : "agi";
  ...
  if (combat) {
    const updatedEvent = await updateActiveGrenadeEvent(combat, nextEvent.id, (entry) => {
      entry.grenadeCookEvasionPenalty = Number(grenadeCookEvasionPenalty ?? 0);
      entry.cookResolved = true;
  ...
  const cookResultUpsert = await upsertGrenadeCookStageMessage(combat, nextEvent, {
...
async function rollCookDamageAtTurnStart(event, { combat = game.combat, promptMessage = null } = {}) {
  let nextEvent = foundry.utils.deepClone(event ?? {});
  if (!nextEvent.damageReady) {
    ui.notifications?.warn("Finish the staged throw, scatter, and cook steps before rolling damage.");
    return null;
  }
  ...
  if (promptMessage) {
    await updateGrenadeCookStageMessage(promptMessage, "grenadeCookDamagePrompt", {
      content: `
          <div class="mythic-evasion-card">
...
async function explodeGrenadeEvent(event) {
  ...
  let blastDamage = Number(event.attackData?.grenadeBlastDamage ?? event.attackData?.damageTotal ?? 0);
  let killDamage = Number(event.attackData?.grenadeKillDamage ?? (blastDamage * 2));
  let blastPierce = Number(event.attackData?.grenadeBlastPierce ?? event.attackData?.damagePierce ?? 0);
  ...
  if (!hasUsableBlast || !hasUsableKill) {
    if (damageFormula) {
      ...
      damageFormula = String(rolled?.damageFormula ?? damageFormula ?? "").trim();
    }
  }
...
async function processCombatGrenadeCooks(combat) {
  ...
  const dueEvents = activeEvents.filter((event) => {
    ...
    if (String(event.combatId ?? "").trim() !== String(combat.id ?? "").trim()) return false;
    const detonateAt = event.detonateAt ?? {};
    const detonateRound = Math.max(0, Math.floor(Number(detonateAt.round ?? 0)));
    const detonateTurn = Math.max(0, Math.floor(Number(detonateAt.turn ?? 0)));
    return currentRound > detonateRound || (currentRound === detonateRound && currentTurn >= detonateTurn);
  });
  ...
  for (const entry of dueEvents) {
    ...
    const actorToken = getGrenadeOwnerToken(nextEvent);
    if (actorToken?.center) {
      const syncedEvent = await updateActiveGrenadeEvent(combat, nextEvent.id, (activeEntry) => {
...
export function registerMythicDocumentAndChatHooks({
  ...
  Hooks.on("getSceneControlButtons", (controls) => {
    ...
    const tileControls = Array.isArray(controls)
      ? controls.find((control) => String(control?.name ?? "").trim() === "tiles")
      : (controls.tiles ?? Object.values(controls).find((control) => String(control?.name ?? "").trim() === "tiles"));
    ...
    if (existingTools.some((tool) => String(tool?.name ?? "").trim() === "mythic-clear-ordnance-markers")) return;

    const nextToolOrder = existingTools.reduce((maxOrder, tool) => {
      const toolOrder = Number(tool?.order ?? 0);
      return Number.isFinite(toolOrder) ? Math.max(maxOrder, toolOrder) : maxOrder;
    }, 0) + 1;
    ...
    const clearOrdnanceTool = {
      ...
      onChange: async (_event, active) => {
        ...
        const markerIds = getClearableOrdnanceMarkerIds(scene);
        if (!markerIds.length) {
          ui.notifications?.info("No Mythic / 100DOS ordnance markers were found on this scene.");
          return;
        }
        ...
        ui.notifications?.info(`Cleared ${cleared.clearedCount} Mythic / 100DOS ordnance marker${cleared.clearedCount === 1 ? "" : "s"}.`);
      }
    };
  ...
  Hooks.on("createTile", (tileDocument) => {
    ...
    if (String(tileDocument.parent?.id ?? "") !== String(canvas?.scene?.id ?? "")) return;
  ...
  Hooks.on("deleteTile", (tileDocument) => {
    if (!isGrenadeMarkerTileDocument(tileDocument)) return;
    const tileObject = canvas?.tiles?.placeables?.find((entry) => String(entry.id ?? "") === String(tileDocument.id ?? "")) ?? null;
    if (tileObject) {
      stopGrenadeMarkerAnimation(tileObject);
    }
  });
  ...
  Hooks.on("deleteToken", (tokenDocument) => {
    void syncCookEventPositionsForTokenDocument(tokenDocument, { clearTokenId: true });
  });
  ...
  Hooks.on("renderChatMessageHTML", async (message, htmlElement) => {
    ...
    if (attackData?.isDeferredCookStart && !isGrenadeExplosionMessage) {
      const grenadeData = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeData");
    ...
    if (grenadeControl && game.user.isGM) {
      ...
      const controlCombat = controlCombatId && String(game.combat?.id ?? "").trim() === controlCombatId
        ? game.combat
        : null;
      const canUseMessageState = Boolean(grenadeControl.outsideCombat || !controlCombatId);
      ...
      if (eventId && (controlCombat || canUseMessageState)) {
        ...
        `;
        scatterPanel.querySelector(".mythic-grenade-scatter-btn")?.addEventListener("click", async () => {
          await rollAndApplyGrenadeScatter(controlCombat, eventId, { controlMessage: message });
        });
        scatterPanel.querySelector(".mythic-grenade-move-btn")?.addEventListener("click", async () => {
          await moveGrenadeMarkerManually(controlCombat, eventId, { controlMessage: message });
        });
        cardEl.appendChild(scatterPanel);
      }
    }

    const grenadeExplosionControls = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeExplosionControls");
    if (grenadeExplosionControls && attackData && game.user.isGM) {
      const msgId = message.id;
      const explosionControlData = grenadeExplosionControls && typeof grenadeExplosionControls === "object"
        ? grenadeExplosionControls
        : {};
      const markerId = String(explosionControlData.markerId ?? "").trim();
      const sceneId = String(explosionControlData.sceneId ?? attackData.sceneId ?? canvas?.scene?.id ?? "").trim();
      const templateIds = normalizeGrenadeExplosionTemplateIds(explosionControlData.templateIds);
      const templateX = Number(explosionControlData.x ?? 0);
      const templateY = Number(explosionControlData.y ?? 0);
      const blastRadius = Math.max(0, Number(explosionControlData.blastRadius ?? attackData.blastRadius ?? 0) || 0);
      const killRadius = Math.max(0, Number(explosionControlData.killRadius ?? attackData.killRadius ?? 0) || 0);
      const canCreateTemplates = blastRadius > 0 || killRadius > 0;
      const canClearMarker = Boolean(markerId) && explosionControlData.markerCleared !== true;
      ...
      panel.querySelector(".mythic-create-grenade-templates-btn")?.addEventListener("click", async () => {
        ...
        try {
          ...
          const targetScene = game.scenes?.get(String(sceneId ?? "")) ?? canvas?.scene ?? null;
          let tile = null;

          // Try explicit marker id lookup first (if present)
          if (mId && targetScene) {
            try {
              tile = targetScene.getEmbeddedDocument ? targetScene.getEmbeddedDocument("Tile", mId) : (targetScene.tiles && typeof targetScene.tiles.get === "function" ? targetScene.tiles.get(mId) : null);
            } catch (err) {
              tile = (targetScene.tiles && typeof targetScene.tiles.get === "function") ? targetScene.tiles.get(mId) : null;
            }
            if (!tile && canvas?.scene && canvas.scene.id === String(targetScene.id ?? "")) {
          ...
          if (!tile && targetScene) {
            try {
              const eventIdForSearch = String(message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeEventId") ?? "").trim();
              if (eventIdForSearch) {
          ...
          if (tile) {
            foundMarkerId = String(tile.id ?? tile._id ?? foundMarkerId ?? "").trim() || foundMarkerId;
        ...
        await message.setFlag("Halo-Mythic-Foundry-Updated", "grenadeExplosionControls", {
          ...explosionControlData,
          markerId: explosionControlData.markerId || foundMarkerId || "",
          templateIds: normalizeGrenadeExplosionTemplateIds([
            ...templateIds,
            ...(Array.isArray(createdTemplates?.templateIds) ? createdTemplates.templateIds : [])
          ])
        });
        await message.render(true);
        ui.notifications?.info(`Created ${createdCount} grenade radius template${createdCount === 1 ? "" : "s"}.`);
      });
      panel.querySelector(".mythic-clear-grenade-marker-btn")?.addEventListener("click", async () => {
        const cleared = await clearGrenadeExplosionVisualsByData({ sceneId, markerId, templateIds });
        ...
        if (cleared.templateCount > 0) {
          clearedParts.push(`${cleared.templateCount} radius template${cleared.templateCount === 1 ? "" : "s"}`);
        }
    ...
    const grenadeCookPrompt = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookPrompt");
    if (grenadeCookPrompt && canInteractWithGrenadeCookStage(message, grenadeCookPrompt.ownerActorId)) {
      ...
      const promptCombat = combatId && String(game.combat?.id ?? "").trim() === combatId
        ? game.combat
        : null;
      if (eventId && !promptResolved && (promptCombat || !game.combat)) {
        ...
        `;
        promptPanel.querySelector(".mythic-resolve-cook-btn")?.addEventListener("click", async () => {
          const currentEvent = await getGrenadeEventForControl(promptCombat, eventId, message);
          if (!currentEvent) {
    ...
    const grenadeCookThrowResult = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookThrowResult");
    if (grenadeCookThrowResult && canInteractWithGrenadeCookStage(message, grenadeCookThrowResult.ownerActorId)) {
      ...
      if (eventId && promptCombat) {
        ...
        `;
        promptPanel.querySelector(".mythic-resolve-cook-scatter-btn")?.addEventListener("click", async () => {
          const currentEvent = await getGrenadeEventForControl(promptCombat, eventId, message);
          if (!currentEvent) {
            await updateGrenadeCookStageMessage(message, "grenadeCookThrowResult", {
        ...
        promptPanel.querySelector(".mythic-resolve-cook-test-btn")?.addEventListener("click", async () => {
          const currentEvent = await getGrenadeEventForControl(promptCombat, eventId, message);
          if (!currentEvent) {
            await updateGrenadeCookStageMessage(message, "grenadeCookThrowResult", {
              updates: { resolved: true }
    ...
    const grenadeCookScatterResult = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookScatterResult");
    if (grenadeCookScatterResult && canInteractWithGrenadeCookStage(message, grenadeCookScatterResult.ownerActorId)) {
      ...
      if (eventId && promptCombat) {
        ...
        `;
        promptPanel.querySelector(".mythic-cook-scatter-move-btn")?.addEventListener("click", async () => {
    ...
    const grenadeCookResult = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookResult");
    if (grenadeCookResult && canInteractWithGrenadeCookStage(message, grenadeCookResult.ownerActorId)) {
      ...
      if (eventId && promptCombat) {
        ...
        `;
        promptPanel.querySelector(".mythic-cook-result-move-btn")?.addEventListener("click", async () => {
          await moveGrenadeMarkerManually(promptCombat, eventId);
        });
        cardEl.appendChild(promptPanel);
      }
    ...
    const grenadeCookDamagePrompt = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookDamagePrompt");
    if (grenadeCookDamagePrompt && canInteractWithGrenadeCookStage(message, grenadeCookDamagePrompt.ownerActorId)) {
      ...
      if (eventId && !promptResolved && (promptCombat || !game.combat)) {
        ...
        `;
        promptPanel.querySelector(".mythic-roll-cook-damage-btn")?.addEventListener("click", async () => {
          const currentEvent = await getGrenadeEventForControl(promptCombat, eventId, message);
          if (!currentEvent) {
            await updateGrenadeCookStageMessage(message, "grenadeCookDamagePrompt", {
              updates: { resolved: true }
            });
            return;
          }
    ...
    const grenadeData = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeData");
    if (attackData && (game.user.isGM || message.isAuthor) && attackData.isTimedDetonation && !isGrenadeExplosionMessage) {
      const preferredAction = String(attackData.grenadeArmAction ?? "").trim().toLowerCase();
      ...
      if (!grenadeData?.armed) {
        const resolvedThrowRange = Math.max(0, Math.floor(Number(attackData.throwRangeMeters ?? attackData.throwRangeMaxMeters ?? 0)));
        const defaultThrowRange = Math.max(0, Math.floor(Number(attackData.throwRangeMaxMeters ?? 0)));
        ...
        grenadePanel.querySelectorAll(".mythic-grenade-action-btn").forEach((btn) => {
          btn.addEventListener("click", async () => {
            const action = String(btn.dataset.action ?? "throw").trim().toLowerCase();
            if (typeof createGrenadeEvent === "function") {
        ...
        const grenadeAction = String(grenadeData?.action ?? attackData.grenadeArmAction ?? "").trim().toLowerCase();
        ...
        if (showImmediate) {
          ...
          grenadePanel.querySelector(".mythic-roll-timed-damage-btn")?.addEventListener("click", async () => {
            const currentEvent = await getGrenadeEventForControl(null, eventId, message);
            if (!currentEvent) {
              ui.notifications?.warn("No grenade event available to resolve.");
              return;
            }
  ...
  try {
    if (!window.__mythicCookDelegatesInstalled) {
      document.addEventListener("click", async (ev) => {
        const btn = ev.target.closest?.(".mythic-resolve-cook-btn, .mythic-resolve-cook-test-btn, .mythic-resolve-cook-scatter-btn");
        if (!btn) return;
        ev.preventDefault();
        ev.stopPropagation();

        const messageEl = btn.closest?.("[data-message-id], .chat-message, .message");
        ...
        try {
          ...
          if (btn.classList.contains("mythic-resolve-cook-btn")) {
            ...
            const eventId = String(grenadeCookPrompt.eventId ?? "").trim();
            const combatId = String(grenadeCookPrompt.combatId ?? "").trim();
            const promptCombat = combatId && String(game.combat?.id ?? "").trim() === combatId ? game.combat : null;
            ...
            await resolveCookThrowAtTurnStart(currentEvent, { combat: promptCombat, promptMessage: message });
            return;
          }

          // Resolve Scatter (from the throw result card)
          if (btn.classList.contains("mythic-resolve-cook-scatter-btn") || btn.classList.contains("mythic-resolve-cook-test-btn")) {
            const grenadeCookThrowResult = message.getFlag("Halo-Mythic-Foundry-Updated", "grenadeCookThrowResult");
            ...
            const currentEvent = await getGrenadeEventForControl(promptCombat, eventId, message);
            if (!currentEvent) {
              await updateGrenadeCookStageMessage(message, "grenadeCookThrowResult", { updates: { resolved: true } });
              return;
            }

            if (btn.classList.contains("mythic-resolve-cook-scatter-btn")) {
              await resolveCookScatterAtTurnStart(currentEvent, { combat: promptCombat, throwResultMessage: message });
            } else {
              if (currentEvent.scatterRequired && !currentEvent.scatterResolved) {
                ui.notifications?.warn("Resolve scatter first.");
                return;
              }
              await resolveCookTestAtTurnStart(currentEvent, { combat: promptCombat });
            }
            return;
          }
        } catch (err) {
          console.error("[mythic-system] delegated cook control handler error:", err);
          ui.notifications?.error("An error occurred handling cooked grenade controls. See console for details.");
        }
      }, true);
      window.__mythicCookDelegatesInstalled = true;
    }
  } catch (err) {
    console.warn("[mythic-system] failed to install delegated cook handlers", err);
  }
}
